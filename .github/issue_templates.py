...
for template in templates:
  with open(f'ISSUE_TEMPLATE/{template.name}.md', 'w') as f:
    ...
    print(f'{next(action)}. you have picked the right category for the issue in the previous screen.', file=f)
    ...
    if template.name in ['Key_generation', 'Export']:
      print(f'{next(action)}. you have included an error-report ID here generated by reproducing the problem, selecting the problematic reference(s), right-clicking, and submitting an BBT error report from that popup menu', file=f)
      print(f'{next(action)}. you have included the actual output you got from exporting the items you sent in the debug log, not simplified fictional output, and a sample of you you want it to look, again based on the actual items under consideration', file=f)
    ...
    print("Picking the right issue category is really important. Each category (" + ', '.join([f'`{template.name.replace("_", " ")}`' for template in templates]) + ") has different instructions for gathering the data necessary required to resolve the issue you are experiencing", file=f)
    print('', file=f)
