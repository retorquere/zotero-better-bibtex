...
for template in templates:
  with open(f'ISSUE_TEMPLATE/{template.name}.md', 'w') as f:
    ...
    print('1. in the Zotero addons screen you can see that you have the latest release of BBT (https://github.com/retorquere/zotero-better-bibtex/releases/latest) and Zotero installed', file=f)

    if template.name in ['Key_generation', 'Import', 'Export', 'General_error']:
      print('1. you are posting a single bug or feature request', file=f)
    elif template.name in ['Question']:
      print('1. you are posting a single question', file=f)
  
    print('1. you have included a descriptive subject of the problem', file=f)
    print('1. you are available for follow-up questions and testing', file=f)

    if template.name in ['Import']:
      print('1. you have attached a copy of the BibTeX you were trying to import to this issue.', file=f)

    if template.name in ['Key_generation', 'Export']:
      print('1. you have included an error-report ID here generated by selecting the problematic reference(s), right-clicking, and submitting an BBT error report from that popup menu', file=f)
    elif template.name in ['Import', 'General_error']:
      print('1. you have included an error-report ID here generated by restarting Zotero with debugging enabled (Help -> Debug Output Logging -> Restart with logging enabled), reproducing your problem, and selecting "Report Better BibTeX error" from the help menu', file=f)
