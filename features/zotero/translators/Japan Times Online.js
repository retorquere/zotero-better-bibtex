{
	...
	"lastUpdated": "2014-04-03 17:40:07"
}
...
detect           : FW.Xpath('//div[@class="main"]//header/hgroup/h1'),
title            : FW.Xpath('//div[@class="main"]//header/hgroup/h1').text().trim(),
attachments      : [{ url: FW.Url(),
  title:  "Japan Times Snapshot",
  type: "text/html" }],
creators         : FW.Xpath('//div[@class="main"]//h5[@class="writer"]/a/text()').text().capitalizeTitle(true	).cleanAuthor("author"),
date             : FW.Xpath('//li[@class="post_time"]/time').text(),
abstractNote	 : FW.Xpath('//meta[@name="description"]/@content').text().trim(),
language		 : "en-US",
ISSN			 : "0447-5763",
url				 : FW.Url().remove(/#[^\/]+$/),
...
FW.MultiScraper({
itemType         : 'multiple',
detect           : FW.Xpath('//a[@class="gs-title"]'),
choices          : {
  titles :  FW.Xpath('//a[@class="gs-title" and contains(@href, "/20")]').text(),
  urls    :  FW.Xpath('//a[@class="gs-title"  and contains(@href, "/20")]').key("href")
}
});
/** BEGIN TEST CASES **/
var testCases = [
	...
	{
		"type": "web",
		"url": "http://www.japantimes.co.jp/life/2013/02/05/digital/expat-game-developers-have-an-unfair-advantage-in-japan/#at_pco=cfd-1.0",
		"items": [
			{
				"itemType": "newspaperArticle",
				"creators": [
