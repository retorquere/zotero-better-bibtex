{
	...
	"lastUpdated": "2013-06-09 16:04:13"
}
...
detect : FW.Xpath('//body/h2'),
title : FW.Xpath('//body/h2').text().trim(),
attachments : [{
  url : FW.Url(),
...
creators : FW.Xpath('//table/tbody/tr/td[contains(text(), "Author")]/following-sibling::td').text().remove(/\[.+?\]/g).split(/\s*,\s*/).cleanAuthor("author"),
version : FW.Xpath('//table/tbody/tr/td[contains(text(), "Version")]/following-sibling::td').text(),
abstractNote : FW.Xpath('//body/p').text(),
date : FW.Xpath('//table/tbody/tr/td[contains(text(), "Published")]/following-sibling::td').text(),
archive : FW.Xpath('//tr/td[@id="fileinfotpl_src"]/following-sibling::td|//tr/td[@id="fileinfotpl_art_gallery"]/following-sibling::td').text().trim().remove(/\n.+/g),
rights : FW.Xpath('//table/tbody/tr/td[contains(text(), "License")]/following-sibling::td').text(),
});



/**display by date/name*/
FW.MultiScraper({
itemType : "multiple",
detect : FW.Url().match(/web\/packages\/available_packages_by_(date|name)\.html/),
choices : {
  titles : FW.Xpath('//tr/td/a[contains(@href, "/web/packages/")]').text(),
  urls : FW.Xpath('//tr/td/a[contains(@href, "/web/packages/")]').key('href').text()
}
});
/** BEGIN TEST CASES **/
var testCases = [
