...
3. ternaries: `(auth ? year : title)` (if `auth` returns any text, use `year`, otherwise use `title`)

these can be combined, eg `(auth || shorttitle || year ? auth + title : year || title)`, but subformulae cannot appear in parameters, so `title.select(auth ? 3 : 4)` is not valid.
