{
	"translatorID": "c2f5f114-c255-4cf7-a7b8-d5c95e82291f",
	"label": "Hoovers",
	"creator": "Sebastian Karcher",
	"target": "^https?://(www\\.|subscriber\\.)?hoovers\\.com",
	"minVersion": "3.0",
	"maxVersion": "",
	"priority": 100,
	"inRepository": true,
	"translatorType": 4,
	"browserSupport": "g",
	"lastUpdated": "2016-11-01 17:38:21"
}
...
/*
   Hoover's Translator Copyright (C) 2014 Sebastian Karcher
...
var industryDetails = "&industryHeader=on&reportType=custom&generalInfo=on&description=on&competitiveLandscape=on&salesMarketing=on&regIntIssues=on&productsOpsTech=on&financeRegulation=on&humanResources=on&industryCodes=on&naics=on&sic=on&companyList=on&forecastAndRatings=on&financials=on&econStatsInfo=on&valuationMultiples=\
on&forecast=on&growthRate=on&trendsOpp=on&trends=on&opportunities=on&businessInformation=on&executiveInsights=on&ceo=on&cio=on&vpSales=on&cfo=on&hr=on&businessChallenges=on&criticalIssues=on&otherChallenges=on&callPrepQuestions=on&conversationStarters=on&regulationRDImportExport=\
on&finAnalysis=on&opsProdFacilities=on&orgManagement=on&bizTech=on&custMarketing=on&recentDevelopments=on&qtrUpdate=on&websites=on&indicators=on&glossary=on&fastfacts=on"

/** Articles */
FW.Scraper({
itemType         : 'report',
detect           : FW.Xpath('//body[@id="industryOverview" or @id="companyOverview"]//h1'),
title            : FW.Xpath('//div[@id="content"]//h1/text()').text().trim(),
attachments      : [{ url: FW.Url(),
  title:  "Hoover's Snapshot",
  type: "text/html" }],
creators         : FW.Xpath('//p[@class="editorByline"]/text()').text().remove(/.+covered by\s*/).trim().cleanAuthor("author"),
date             : FW.Xpath('//h2[contains(text(), "Quarterly Industry Update")]').text().remove(/Quarterly Industry Update\s*-\s*/).trim(),
institution	 	 : "Hoover's, Inc.",
place			 : "Austin, TX",
hooks : { "scraperDone": function  (item,doc, url) {
	//grab full report, differs by category
	var reporturl;
	if (url.indexOf("industryId=")!=-1){
		item.reportType = "Hoover's Industry Report";
		reporturl = url.replace(/[^\/]+\.html/, "printReport.html")+ industryDetails;
	}
	else if (url.indexOf("companyId")!=-1){
		item.reportType = "Hoover's Company Report";
		reporturl = url.replace(/[^\/]+\.html/, "quickReport.html");
	}
	
	if (reporturl){
		item.attachments.push({url: reporturl, title: "Hoover's Report", mimeType: "text/html"})
	}
...
choices          : {
  titles :  FW.Xpath('//tbody[contains(@id, "SearchResults")]//a[contains(@href, "companyId") or contains(@href, "industryId")]').text().trim(),
  urls    :  FW.Xpath('//tbody[contains(@id, "SearchResults")]//a[contains(@href, "companyId") or contains(@href, "industryId")]').key("href")
}
});
