{
	...
	"label": "Eurogamer",
	"creator": "czar",
	"target": "^https?://(www\\.)?eurogamer\\.net",
	"minVersion": "3.0",
	"maxVersion": "",
	"priority": 100,
	"inRepository": true,
	...
	"lastUpdated": "2016-11-01 18:32:59"
}
...
attachments      : [{ url: FW.Url(),
  title:  "Eurogamer snapshot",
  type: "text/html" }],
creators         : FW.Xpath('//p[@class="byline"]/a').text().cleanAuthor("author"),
date             : FW.Xpath('//span[@itemprop="datePublished"]/@content').text(),
blogTitle        : "Eurogamer",
language         : "en-UK",
...
FW.MultiScraper({
itemType         : 'multiple',
detect           : FW.Url().match(/\/games\//), // games pages
choices          : {
  titles :  FW.Xpath('//div[@class="headlines"]/ul/li/div/h2/a').text().trim(),
  urls    :  FW.Xpath('//div[@class="headlines"]/ul/li/div/h2/a').key("href").trim()
}
});

FW.MultiScraper({
itemType         : 'multiple',
detect           : FW.Url().match(/\/search\.php/), // search pages
choices          : {
  titles :  FW.Xpath('//a[@class="gs-title"]').text().trim(),
  urls    :  FW.Xpath('//a[@class="gs-title"]').key("href")
}
});/** BEGIN TEST CASES **/
var testCases = [
	{
		"type": "web",
		"url": "http://www.eurogamer.net/articles/2016-06-13-standalone-witcher-card-game-gwent-announced",
		"items": [
			{
				"itemType": "blogPost",
