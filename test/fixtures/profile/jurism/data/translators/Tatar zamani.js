{
	"translatorID": "eb4cb148-b050-470c-9686-77d722efdbd7",
	"label": "Tatar zamanı",
	"creator": "Avram Lyon",
	"target": "^https?://(www\\.)?tatartime\\.com/",
	"minVersion": "1.0",
	"maxVersion": "",
	"priority": 100,
	"inRepository": true,
	...
	"lastUpdated": "2017-01-01 16:59:20"
}
...
/*
   Tatar Time Translator
   Copyright (C) 2012 Avram Lyon, ajlyon@gmail.com
...
FW.Scraper({ itemType         : 'newspaperArticle',
			 detect           : FW.Xpath('//div[@class="post"]/h1'),
			 title            : FW.Xpath('//div[@class="post"]/h1').text().trim(),
			 attachments      : [
				{
					url :	FW.Url(),
					type :	"text/html",
					title: 	"Tatar zamanı fulltext"
				}],
			 section		  : FW.Xpath('//div[@class="post-meta"]/a[@rel="category"]').text(),
			 creators         : FW.Xpath('//div[@class="post-meta"]/a[@rel="author"]').text().remove(/^[^\n]*\n/).remove(/\n.*/).cleanAuthor("author"),
			 date             : FW.Xpath('//div[@class="post-meta"]/text()[last()]').text().remove(/^[\s\||]*/),
			 publicationTitle : "Tatar zamanı"
});

/* This can be thrown off by articles that the user doesn't have access to... */
/** Search results */
FW.MultiScraper({ itemType  : "multiple",
				  detect    : FW.Url().match(/\?s=.+/),
				  choices : {
			  	titles    : FW.Xpath('//div[@class="homepost"]//h3/a').text(),
				  	urls      : FW.Xpath('//div[@class="homepost"]//h3/a').key('href').text()
				  }
});
...
var testCases = [
	{
		"type": "web",
		"url": "http://www.tatartime.com/?s=%D0%9D%D0%B0%D0%B8%D0%BB+%D0%9D%D3%99%D0%B1%D0%B8%D1%83%D0%BB%D0%BB%D0%B8%D0%BD",
		"items": "multiple"
	},
	{
		"type": "web",
		"url": "http://www.tatartime.com/?p=10398",
		"items": [
			{
				...
				"url": "http://www.tatartime.com/?p=10398",
				"date": "13.08.2012 17:42",
				"publicationTitle": "Tatar zamanı",
				"section": "Идел-урал",
				"title": "Наил Нәбиуллин: Хизб ет-тәхрир Татарстанга каршы эшли һәм алар белән башкача бәйләнмим",
				"libraryCatalog": "Tatar zamanı",
				"accessDate": "CURRENT_TIMESTAMP",
				"shortTitle": "Наил Нәбиуллин"
			}
		]
	}
]
