...
var Translator = {
  ...
  header: {"translatorID":"f4b52ab0-f878-4556-85a0-c7aeedd09dfc","label":"Better CSL JSON","creator":"Emiliano heyns","target":"json","minVersion":"4.0.27","maxVersion":"","displayOptions":{"Keep updated":false},"translatorType":2,"browserSupport":"gcsv","configOptions":{"serializationCache":true},"inRepository":false,"priority":100,"lastUpdated":"2017-09-05 07:26:36"},
...
Exporter = __webpack_require__(/*! ./csl/csl.coffee */ 66);

Exporter.prototype.serialize = function(csl) {
  return JSON.stringify(csl);
};

Exporter.prototype.flush = function(items) {
  var item;
  return "[\n" + ((function() {
    var i, len, results;
    results = [];
...
var CSLExporter, Citekey, Exporter, ValidCSLTypes, date2csl, debug, parseDate,
...
ValidCSLTypes = ['article', 'article-magazine', 'article-newspaper', 'article-journal', 'review', 'review-book', 'bill', 'broadcast', 'dataset', 'figure', 'graphic', 'interview', 'legislation', 'legal_case', 'map', 'motion_picture', 'musical_score', 'patent', 'post', 'post-weblog', 'personal_communication', 'song', 'speech', 'treaty', 'webpage', 'book', 'chapter', 'entry', 'entry-dictionary', 'entry-encyclopedia', 'manuscript', 'pamphlet', 'paper-conference', 'report', 'thesis'];

date2csl = function(date) {
  var csl, ref;
  switch (date.type) {
    case 'open':
      return [0];
  ...
  if ((ref = date.type) !== 'date' && ref !== 'open') {
...
CSLExporter = (function() {
  function CSLExporter() {
    this.Exporter = new Exporter();
  }

  CSLExporter.prototype.doExport = function() {
    var cached, citekey, creator, creators, csl, fields, i, item, items, j, k, kind, l, len, len1, len2, m, name, ref, ref1, ref2, ref3, ref4, ref5, swap, v, value;
    items = [];
    while (item = Zotero.nextItem()) {
      ...
      Zotero.BetterBibTeX.simplifyFields(item);
      item.extra = Citekey.get(item.extra).extra;
      fields = this.Exporter.extractFields(item);
      if (item.accessDate) {
      ...
      if (ref = item.cslType, indexOf.call(ValidCSLTypes, ref) >= 0) {
        csl.type = item.cslType;
      }
      delete csl.authority;
      if (item.__type__ === 'videoRecording' && csl.type === 'video') {
        csl.type = 'motion_picture';
      }
      if (csl.issued && item.date) {
        csl.issued = parseDate(item.date);
      }
      ...
      for (name in fields) {
        ...
        switch (this.Exporter.CSLVariables[name].type) {
          case 'date':
      ...
      if ((ref2 = csl.type) === 'motion_picture' || ref2 === 'broadcast') {
        swap.author = 'director';
      }
      for (k in swap) {
        v = swap[k];
        ref3 = [csl[v], csl[k]], csl[k] = ref3[0], csl[v] = ref3[1];
      }
      citekey = csl.id = item.citekey;

      /* Juris-M workarounds to match Zotero as close as possible */
      ref4 = ['author', 'editor', 'director'];
