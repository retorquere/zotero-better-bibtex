{
	"translatorID": "58ae38d2-80d3-4569-81eb-9cefc43ac5b4",
	"label": "EurasiaNet",
	"creator": "Avram Lyon",
	...
	"lastUpdated": "2012-03-04 20:52:45"
}
...
attachments :		[
				{	url: FW.Url().replace(/node/,"print"),
					title:  "EurasiaNet Printable",
					type: "text/html" }
					// Video? Not yet.
/*				{	url: FW.Xpath('//object[@id="videofield-embedded_api"]/param[@name="flashvars"]').key("value").match(/"url":"([^"])"/,0),
					title:  "EurasiaNet Video",
					type: "video/x-flv" } */
					],
// here, we use the replace(..) to break names on &nbsp;
creators         : FW.Xpath('//div[@class="submitted"]/span[@class="authors"]/a').text().replace(/\s/," ").cleanAuthor("author"),
date             : FW.Xpath('//div[@class="submitted"]/span[@class="timestamp"]').text().replace(/-.+/, ""),
...
var testCases = [
	...
	{
		"type": "web",
		"url": "http://www.eurasianet.org/search/node/tymoshenko",
		"items": "multiple"
	}
]
/** END TEST CASES **/