{
	...
	"target": "^https?://(www|recherche)\\.(cyberpresse|lapresse)\\.ca",
	"minVersion": "2.1.9",
	"maxVersion": "",
	"priority": 100,
	"inRepository": true,
	...
	"lastUpdated": "2012-05-02 19:20:01"
}
...
/*
	***** BEGIN LICENSE BLOCK *****
	...
	La Presse Translator - Copyright © 2012 Sebastian Karcher 
...
detect           : FW.Xpath('//div[contains(@class, "article-header")]/h1'),
title            : FW.Xpath('//div[contains(@class, "article-header")]/h1').text().trim(),
...
FW.MultiScraper({
itemType         : 'multiple',
detect           : FW.Url().match(/recherche\.cyberpresse/),
choices          : {
  titles :  FW.Xpath('//li/h3/a').text().trim(),
  urls    :  FW.Xpath('//li/h3/a').key("href")
}
});/** BEGIN TEST CASES **/
var testCases = [
	{
