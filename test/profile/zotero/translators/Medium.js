{
	...
	"lastUpdated": "2014-06-01 20:47:42"
}
...
FW.Scraper({
itemType         : 'webpage',
detect           : FW.Xpath('//header/h1[@class="post-title"]'),
title            : FW.Xpath('//header/h1[@class="post-title"]').text(),
attachments      : [{
  url: FW.Url(),
  title: "Medium Snapshot",
  type: "text/html"}],
creators         : FW.Xpath('//div[contains(@class, "metabar")]//a[contains(@class, "post-meta-author")]/span').text().cleanAuthor("author"),
date             : FW.Xpath('//time[@class="post-date"]').text(),
abstractNote     : FW.Xpath('//meta[@property="og:description"]/@content').text(),
websiteTitle     : FW.Xpath('//span[contains(@class, "post-meta-collection")]/a').text().prepend("Medium: "),
hooks : { "scraperDone": function  (item,doc, url) {
	 var subtitle = ZU.xpathText(doc, '//header/h2[contains(@class, "subtitle")]');
	 if (subtitle){
	 	if (item.title.search(/[\?!:]$/)!=-1){
...
//we migh have to run monitor.DOM.changes for search results, but let's leave this as is for the time being. 
FW.MultiScraper({
itemType         : 'multiple',
detect           : FW.Xpath('//div[contains(@class, "bucket")]//h3[@class="post-item-title"]'),
choices          : {
  titles :  FW.Xpath('//div[contains(@class, "bucket")]//h3[@class="post-item-title"]/a').text().trim(),
  urls    :  FW.Xpath('//div[contains(@class, "bucket")]//h3[@class="post-item-title"]/a').key("href")
}
});
/** BEGIN TEST CASES **/
var testCases = [
		...
		"items": [
			{
				...
				"date": "April 24, 2014",
				"title": "What If the Feature Is the Bug? Election monitoring, new power of social media and old power of structural power",
				"websiteTitle": "Medium: The Message",
				"libraryCatalog": "Medium",
