{
	...
	"lastUpdated": "2013-12-11 21:31:13"
}
...
detect           : FW.Url().match(/(\/basma)?\/[0-9-]+\/\w+|\/[0-9-]+\w+\/\w+/),
title            : FW.Xpath('//h1[@class="entry-title"]').text().trim(),
attachments      : [{ url: FW.Url(),
  title:  "Безнең гәҗит Snapshot",
  type: "text/html" }],
creators         : FW.Xpath('//div[@class="entry-meta"]/p[@class="authors"]//a')
					.text().replace(/^.*Килгән\sхатлардан.*$/, "").cleanAuthor("author"),
date             : FW.Xpath('//div[@class="entry-meta"]/p[@class="post-category"]').text().replace(/(\d{4}),\s№\s?\d+\s\((.*)\)/,"$2 $1"),
issue             : FW.Xpath('//div[@class="entry-meta"]/p[@class="post-category"]').text().match(/№\s?(\d+)/,1),
...
FW.MultiScraper({
itemType         : 'multiple',
detect           : FW.Url().match(/\/rubric\/|\?s=/),
choices          : {
  titles :  FW.Xpath('//h2[@class="entry-title"]/a[contains(@href, "/basma/")]').text().trim(),
  urls    :  FW.Xpath('//h2[@class="entry-title"]/a[contains(@href, "/basma/")]').key("href")
}
});/** BEGIN TEST CASES **/
var testCases = [
	{
		"type": "web",
		"url": "http://beznen.ru/basma/2011-41/kaskali-cencal/",
		"items": [
			{
				"itemType": "newspaperArticle",
				"creators": [
				...
				"url": "http://beznen.ru/basma/2011-41/kaskali-cencal/",
				"date": "19 октябрь 2011",
				"issue": "41",
				"language": "татарча",
				"publicationTitle": "Безнең гәҗит",
				"title": "Каскалы җәнҗал",
				"libraryCatalog": "Безнең гәҗит",
	...
	{
		"type": "web",
		"url": "http://beznen.ru/?s=%D0%B1%D0%B8%D1%80%D0%B3%D3%99%D0%BD",
		"items": "multiple"
	}
]
/** END TEST CASES **/