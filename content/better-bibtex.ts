  ...
  try {
    ...
    if (translator) {
      if (this.location) {
        ...
        const postscript = OS.Path.join(this._displayOptions.exportDir, 'postscript.js')
        try {
      ...
      if (!this.noWait && Prefs.get('workers') && (!this.location || !this.location.path.startsWith('\\\\'))) {
