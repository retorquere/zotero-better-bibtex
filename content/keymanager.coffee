...
class KeyManager
  pin: co((ids) ->
    ...
    for id in ids
      ...
      continue if citekey.pinned

      try
        debug('KeyManager.pin', id)
        citekey = @update(item)
        debug('KeyManager.pin', id, citekey)
        item.setField('extra', Citekey.set(citekey.extra, citekey))
        debug('KeyManager.pin', item.getField('extra'))
        item.saveTx()
      catch err
  ...
  unpin: co((ids) ->
    ...
    for id in ids
      item = yield getItemsAsync(id)
      citekey = Citekey.get(item.getField('extra'))
      continue unless citekey.pinned

      debug('KeyManager.unpin', id)
