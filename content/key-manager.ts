...
export const KeyManager = new class _KeyManager {
  ...
  public toclipboard(ids: 'selected' | number | number[]): void {
    const itemIDs: number[] = this.expandSelection(ids)
    const citekeys = blink.many(this.keys, { where: { itemID: { in: itemIDs }}}).map(item => item.citationKey)
    toClipboard(citekeys.join(','))
  }
