...
export class PatternFormatter {
  ...
  */
   ...
   *
   * Pinned citation keys in Extra are no longer supported, but existing
  ...
  public _jieba(input: string): string {
    return chinese.enabled?.jieba.cut ? chinese.jieba.cut!(input).join(' ').trim() : input
  }
  ...
  private titleWords(title, nopunct = false): string[] {
    ...
    if (this.item.transliterateMode === 'chinese' && chinese.enabled?.jieba.cut) {
