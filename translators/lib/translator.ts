...
export class Translation { // eslint-disable-line @typescript-eslint/naming-convention,no-underscore-dangle,id-blacklist,id-match
  ...
  private constructor(public collected: Collected, private mode: 'import' | 'export') {
    ...
    try {
      ...
      log.debug(`3020: collected charmap: ${JSON.stringify(this.charmap)} from ${typeof this.collected.preferences.charmap} ${this.collected.preferences.charmap}`)
    }
    catch (err) {
    ...
    log.debug(`3020: collected charmap: ${JSON.stringify(this.charmap)}`)
  }
