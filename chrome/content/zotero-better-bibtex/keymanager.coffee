...
Zotero.BetterBibTeX.keymanager.journalAbbrev = (item) ->
  ...
  return unless Zotero.BetterBibTeX.prefs.get('auto-abbrev')

  if typeof @journalAbbrevCache[item.publicationTitle] is 'undefined'
    styleID = Zotero.BetterBibTeX.prefs.get('auto-abbrev.style')
...
Zotero.BetterBibTeX.keymanager.get = (item, pinmode) ->
  ...
  if not citekey
    pattern = Zotero.BetterBibTeX.prefs.get('citeKeyFormat')
  ...
  if citekey.citeKeyFormat && (pinmode == 'manual' || (Zotero.BetterBibTeX.allowAutoPin() && pinmode == Zotero.BetterBibTeX.prefs.get('pin-citekeys')))
