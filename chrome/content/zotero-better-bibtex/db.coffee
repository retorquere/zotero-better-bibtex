...
Zotero.BetterBibTeX.DB = new class
  ...
  purge: ->
    ...
    Zotero.debug("purge:, allitems=#{JSON.stringify(itemIDs)}, keys=#{JSON.stringify(@keys.data)}, cache=#{JSON.stringify(@cache.data)}, serialized=#{JSON.stringify(@serialized.data)}")
