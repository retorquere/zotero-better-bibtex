...
string
  ...
  / text:("\\" [a-z]+ "\\" [a-zA-Z]+)                                  terminator     &{ return lookup(text, '1'); }{ return lookup(text); }
  / text:("\\acute{\\ddot{\\" [a-z]+ "}}")                                            &{ return lookup(text, '2'); }{ return lookup(text); }
  / text:("\\" [a-zA-Z]+ "{\\?" [0-9a-zA-Z]+ "}" ({\\?[0-9a-zA-Z]+ "})?")             &{ return lookup(text, '3'); }{ return lookup(text); }
  / text:("\\" [0-9a-zA-Z]+)                                           terminator     &{ return lookup(text, '4'); }{ return lookup(text); }
  / text:("\\cyrchar{\\'\\" [a-zA-Z]+ "}")                                            &{ return lookup(text, '5'); }{ return lookup(text); }
  / text:("\\mathrm{" [^}]+ "}")                                                      &{ return lookup(text, '6'); }{ return lookup(text); }
  / text:("\\" [a-z]+ "{" [,\.a-z0-9]+ "}")                                           &{ return lookup(text, '7'); }{ return lookup(text); }
  / text:("\\={\\i}")                                                                 &{ return lookup(text, '8'); }{ return lookup(text); }
  / text:("\\" [=kr] "{" [a-zA-Z] "}")                                                &{ return lookup(text, '9'); }{ return lookup(text); }
  / text:("\\" [\.~\^'`"] "{" [a-zA-Z] "}")                                           &{ return lookup(text, '10'); }{ return lookup(text); }
  / text:("\\u \\i")                                                   terminator     &{ return lookup(text, '11'); }{ return lookup(text); }
  / text:("'" "'"+")                                                                  &{ return lookup(text, '12'); }{ return lookup(text); }
  / text:("\\" [Huvc] " " [a-zA-Z])                                    terminator     &{ return lookup(text, '13'); }{ return lookup(text); }
  / text:("\\" [~\^'`"] "\\" [ij])                                     terminator     &{ return lookup(text, '14'); }{ return lookup(text); }
  / text:("\\" [~\^'`"] "" [a-zA-Z])                                   terminator     &{ return lookup(text, '15'); }{ return lookup(text); }
  / text:("\\" [\.=] "" [a-zA-Z])                                      terminator     &{ return lookup(text, '16'); }{ return lookup(text); }
  / text:("\\" [^a-zA-Z0-9])                                                          &{ return lookup(text, '17'); }{ return lookup(text); }
  / text:("\^" [123])                                                  terminator     &{ return lookup(text, '18'); }{ return lookup(text); }
  / text:("~")                                                                        &{ return lookup(text, '19'); }{ return lookup(text); }
  / text:("\\ddot{\\" [a-z]+ "}")                                                     &{ return lookup(text, '20'); }{ return lookup(text); }
  / text:("\\" [a-z]+ "\\" [a-zA-Z]+)                                  terminator     &{ return lookup(text, '21'); }{ return lookup(text); }
  / text:("\\fontencoding{" [^}]+ "}\\selectfont\\char" [0-9]+)        terminator     &{ return lookup(text, '22'); }{ return lookup(text); }
