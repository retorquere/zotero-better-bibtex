...
doExport = ->
  ...
  while item = Translator.nextItem()
    ...
    if item.date
      locale = []
      locale.push(Translator.locale(item.language)) if item.language
      locale.push(Zotero.BetterBibTeX.locale)
      Translator.moment.locale(locale)
      Translator.debug('locale:', locale)
