...
- `=typeName`: a pseudo-function that aborts the current pattern generation if the Zotero reference type != `typeName`. You can test for multiple typenames at once by separating them with slashes (`[=article/report]`)
