...
file 'chrome/content/zotero-better-bibtex/csl-dateparser.js' => 'Rakefile' do |t|
  cleanly(t.name) do
    ...
    sh "#{NODEBIN}/grasp -i -e 'CSL.DateParser' --replace 'Zotero.BetterBibTeX.CSL.DateParser' #{t.name.shellescape}"
    sh "#{NODEBIN}/grasp -i -e 'CSL.debug' --replace 'Zotero.debug' #{t.name.shellescape}"
    sh "#{NODEBIN}/grasp -i -e 'CSL.DATE_PARTS_ALL' --replace 'Zotero.BetterBibTeX.CSL.DATE_PARTS_ALL' #{t.name.shellescape}"
    sh "#{NODEBIN}/grasp -i -e 'thedate[DATE_PARTS_ALL[i]]' --replace 'thedate[Zotero.BetterBibTeX.CSL.DATE_PARTS_ALL[i]]' #{t.name.shellescape}"
    File.rewrite(t.name){|src| """
      Zotero.BetterBibTeX.CSL = { DATE_PARTS_ALL: ['year', 'month', 'day', 'season'] }
