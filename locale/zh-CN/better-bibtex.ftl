...
better-bibtex_aux-scanner = 
    .label = 从 BibTeX AUX/Markdown 文件扫描引用…
better-bibtex_aux-scanner_replace-collection = 
    .label = 扫描 BibTeX AUX/Markdown 文件并替换此分类中的条目……
...
better-bibtex_error-report_upgrade = { $program } 的最新版本为 <strong>{ $upgrade }</strong>, 当前版本为 <strong>{ $running }</strong>; 请先升级。
better-bibtex_error-report_upgrade_auto = { $program } 将每 { $interval } 从 { $channel } 频道获取更新，上次更新时间是 { $lastUpdate }。
better-bibtex_error-report_upgrade_manual = { $program } 更新已禁用
better-bibtex_error-report_no_items = 该日志不包含样本条目。<b>样本条目不是必须的</b>，但是通常对于创建您将要请求的更改/修复的测试用例来说是必要的。要生成包含样本条目的日志：
better-bibtex_error-report_no_items_cancel = 取消此向导
better-bibtex_error-report_no_items_reproduce = 重现问题
better-bibtex_error-report_no_items_select = 选择出现问题的引用，然后右键单击它们
better-bibtex_error-report_no_items_popup = 从弹出菜单中选择 <i>Better BibTeX</i>
better-bibtex_error-report_no_items_report = 选择 <i>{ better-bibtex_report-errors }</i>
better-bibtex_error-report_better-bibtex_cache = 
    .value = 缓存大小：{ $entries } 个条目。
...
better-bibtex_error-report_items = 
    .label = 条目
better-bibtex-error-send-reminder = 点击 "{ $send }" 将会把调试日志发送到供开发人员查看的安全存储中。
better-bibtex_error-report_post-to-github = 请在 GitHub Issue 上发布一条消息，内容包括此调试日志的 ID、问题描述，以及重现问题所需的步骤。 <strong>只有在 GitHub Issue 中引用了日志 ID，日志才会被查看。</strong>
...
better-bibtex_preferences_advanced_export_workers_cache = 
    .label = 启用后台导出缓存
better-bibtex_preferences_advanced_export_cache-touch = 缓存更新时机
better-bibtex_preferences_advanced_export_cache-touch_drop = 导出时
better-bibtex_preferences_advanced_export_cache-touch_fill = 项目发生变化时
better-bibtex_preferences_advanced_extra-field_as-note = 
    .label = 将“其他”字段理解为note
...
better-bibtex_preferences_advanced_ideographs_chinese = 
    .label = 启用引用公式中的“jieba”/“pinyin”过滤器。消耗大量内存。
better-bibtex_preferences_advanced_ideographs_splitName = 
    .label = 拆分所有中文姓名，除非项目语言是日语且日语支持已启用。
better-bibtex_preferences_advanced_ideographs_japanese = 
    .label = 使用“kuroshiro”将日文姓名/标题转换为罗马音。消耗大量内存。
...
better-bibtex_preferences_citekey_auto-pin-delay = 自动填充引用，等待
...
better-bibtex_preferences_citekey_fold = 
    .label = 强制引用关键词为纯文本
better-bibtex_preferences_citekey_format = 引用关键词格式公式
better-bibtex_preferences_citekey_format_installed = 当前引用关键词格式公式
better-bibtex_preferences_citekey_dynamic = 
    .label = 当项目改变时重新生成引用关键词
better-bibtex_preferences_citekey_uniqueness = 保持引用关键词唯一
better-bibtex_preferences_citekey_uniqueness_case = 
    .label = 查重时忽略大小写
better-bibtex_preferences_citekey_uniqueness_conflict = 若设置的引用与现有条目冲突，这些冲突条目的引用键将被
better-bibtex_preferences_citekey_uniqueness_conflict_change = 
    .label = 添加后缀（导致引用关键词更改）
...
better-bibtex_preferences_export_fields_doi-and-url_both = 
    .label = 全部
better-bibtex_preferences_export_fields_doi-and-url_warning = 大多数 BibTeX 样式不支持 DOI/URL 字段，而对于支持这些字段的样式，许多人使用时会忘记加载所需的“url”宏包，所以请确保您加载了它。 DOI 和 URL 字段是所谓的“verbatim”字段，如果没有加载“url”包编译可能会失败。
...
better-bibtex_preferences_tab_citekey = 引用关键词
...
better-bibtex_startup_journal-abbrev = 加载刊名简称
