...
unused_better-bibtex_startup_installing_translators_new = Better BibTeX：已安装新的翻译器
unused_better-bibtex_startup_installing_translators_new_dont-ask-again = 不再询问
unused_better-bibtex_startup_installing_translators_new_drag-and-drop = 已安装新的 Better BibTeX 翻译器。要在拖放（例如拖放引用）时使用，您需要重启一次以激活它们。
unused_better-bibtex_startup_loading-keys = 加载引用
unused_better-bibtex_startup_loading_databases = 加载数据库
unused_better-bibtex_startup_waiting_for_translators = 正在等待 Zotero 翻译器
unused_better-bibtex_texstudio_not_found = 在 { $path } 中找不到 TeXstudio
unused_better-bibtex_zotero-pane_server_disabled = 已禁用 HTTP 导出
unused_preferences_auto-export_status_running = 运行中
unused_preferences_auto-export_status_scheduled = 已预定
unused_zotero_better-bibtex_csquotes = 将左/右引号替换为 \\enquote{ "{" }...{ "}" } (需要 csquotes)
xpi = 
    .description = 让 Zotero 适合我们 LaTeX 用户。
