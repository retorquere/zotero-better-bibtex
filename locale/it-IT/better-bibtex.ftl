...
better-bibtex_error-report_review =
    I dati di seguito stanno per essere inviati a Better BibTeX per il debug. È possibile controllare i data per vedere se sono presenti informazioni sensibili che non si desidera inviare.
...
unused_better-bibtex_db_corrupt_explanation =
    Si è verificato un errore nell'apertura del tuo database. Questo può indicare che il database è corrotto. L'errore riportato da Zotero è: { $error }.
...
unused_better-bibtex_preferences_advanced_tab_export = Esporta
...
unused_preferences_auto-export_status_running = in esecuzione
unused_preferences_auto-export_status_scheduled = programmato
