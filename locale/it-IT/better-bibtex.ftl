...
better-bibtex_error-report_review = I dati di seguito stanno per essere inviati a Better BibTeX per il debug. È possibile controllare i data per vedere se sono presenti informazioni sensibili che non si desidera inviare.
    
    Se si è d'accordo con l'invio di questi dati, si prega di procedere nella schermata successiva dove sarà indicato l'ID di debug da riportare nell'Issue tracker alla pagina
    
    https://github.com/retorquere/zotero-better-bibtex/issue

better-bibtex_error-report_save = 
  .label = Salva una copia del log
...
unused_better-bibtex_first-run = 
  .title = Ti presentiamo Better BibTeX per Zotero
  .label = Ti presentiamo Better BibTeX per Zotero

unused_better-bibtex_first-run_citekey-format = Le tue chiavi di citazione
unused_better-bibtex_first-run_citekey-format_bbt = 
  .label = Usa il formato predefinito delle chiavi di citazione di BBT


unused_better-bibtex_first-run_citekey-format_explanation = BBT genererà delle chiavi per i tuoi elementi. La generazione è altamente configurabile, ed è possibile escluderla, ma va comunque impostato un parametro di default. È possibile modificare questa scelta nelle preferenze in ogni momento.

unused_better-bibtex_first-run_citekey-format_explanation_bbt = Verranno generate nuove chiavi con il formato predefinito di BBT. Se si desidera un formato diverso, modificarlo nelle preferenze, selezionare tutti gli elementi, fare click con il tasto destro e infine 'Aggiorna'.
unused_better-bibtex_first-run_citekey-format_explanation_example = le chiavi di citazione appariranno come

unused_better-bibtex_first-run_citekey-format_explanation_zotero = Scegli questa opzionie se hai dei documenti che utilizzano delle chiavi generate da Zotero senza l'aiuto di BBT. Questa opzione replicherà fedelmente le chiavi generate da Zotero, includese le limitazioni note del generatore di chiavi di Zotero. Usa questa opzione solo se necessiti di chiavi identiche a quelle generate da Zotero.
unused_better-bibtex_first-run_citekey-format_zotero = 
  .label = Usa il formato predefinito di Zotero per le chiavi di citazione

unused_better-bibtex_first-run_done = Fatto!

unused_better-bibtex_first-run_done_explanation = BBT è pronto per l'uso. Puoi cambiare qualsiasi opzione andando nella finestra delle preferenze e aprendo la scheda BBT.
unused_better-bibtex_first-run_drag-and-drop = Trascina citazioni
unused_better-bibtex_first-run_drag-and-drop_enable = 
  .label = Attiva trascinamento delle citazioni


unused_better-bibtex_first-run_drag-and-drop_explanation = Con BBT, puoi trascinare le citazioni nel tuo editor LaTeX preferito. Vuoi attivare la funzione?
unused_better-bibtex_first-run_exporting = Esportazione con BBT

unused_better-bibtex_first-run_exporting_explanation = L'esportazione con BBT funziona come di consueto in Zotero, ma assicurati di selezionare il formato corretto; al posto di 'BibTeX', seleziona
unused_better-bibtex_first-run_import-strings_enable = 
  .label = Espandi le variabili @string con i titoli delle riviste durante l'importazione


unused_better-bibtex_first-run_import-strings_explanation = Alcuni file bibtex usano delle variabili @string per i titoli delle riviste. Con questa opzione attiva, BBT userà una lista di ID di riviste più comuni per sostituire le variabili con il titolo intero.
unused_better-bibtex_first-run_import_unabbreviate = Espandi le stringhe bibtex e le abbreviazioni dei titoli delle riviste durante l'importazione
unused_better-bibtex_first-run_import_unabbreviate_enable = 
  .label = Svolgi le abbreviazioni dei titoli delle riviste durante l'importazione


unused_better-bibtex_first-run_import_unabbreviate_explanation = Alcuni file bibtex usano delle abbreviazioni nel campo 'journal'. Con questa opzione attiva, BBT cercherà di sostituire alla abbreviazioni il titolo delle riviste per esteso durante l'importazione dei file bibtex.

unused_better-bibtex_first-run_welcome = Better BibTeX for Zotero (o, più comunemente, BBT) vuole rendere Zotero più utile per gli utilizzatori abituali di LaTeX o Markdown. Ci sono un sacco di opzioni configurabili -- vedere le preferenze per credere -- ma iniziamo con delle scelte semplici.
...
better-bibtex_preferences_advanced_import_case-protection_warning = Durante l'importazione, BBT applicherà una protezione delle maiuscole (<span class="nocase">...<span>) ai titoli con delle parole tra {"{"}Graffe{"}"}.
...
better-bibtex_preferences_advanced_import_sentence-case_warning = le voci in Bib(La)TeX devono essere memorizzate con le iniziali maiuscole di ciascuna parola; Zotero suppone che gli elementi siano inseriti con solo l'iniziale della prima parola in maiuscolo.
...
better-bibtex_preferences_export-prefs = 
  .label = Esporta le preferenze di BetterBibTeX...
...
better-bibtex_preferences_export_map-unicode = Nelle conversioni in comandi latex in testo normale:
better-bibtex_preferences_export_map-unicode_conservative = Minimizza il numero di passaggi tra modalità matematica e testuale
  .label = { better-bibtex_preferences_export_map-unicode_conservative }
better-bibtex_preferences_export_map-unicode_creator = Aggiungi parentesi graffe intorno ai caratteri accentati per facilitare il lavoro dei parser latex più semplici
  .label = { better-bibtex_preferences_export_map-unicode_creator }
better-bibtex_preferences_export_map-unicode_math = Preferisci le sostituzioni in modalità matematica
  .label = { better-bibtex_preferences_export_map-unicode_math }
better-bibtex_preferences_export_map-unicode_minimal-packages = Minimizza la richiesta di pacchetti latex aggiuntivi
  .label = { better-bibtex_preferences_export_map-unicode_minimal-packages }
better-bibtex_preferences_export_map-unicode_text = Preferisci le sostituzioni in modalità testuale
...
better-bibtex_preferences_import-prefs = 
  .label = Importa preferenze di BetterBibTeX...
...
better-bibtex_zotero-pane_column_citekey = Chiave di citazione
...
unused_better-bibtex_auto-export_too-long_suggest-idle = Per ridurre al minimo i disagi dovuti a esportazioni automatiche di lunga durata, si consiglia di impostare le esportazioni automatica a "quando inattivo" invece di "a ogni modifica".
unused_better-bibtex_auto-export_too-long_title = Esportazioni automatiche di lunga durata
unused_better-bibtex_db_corrupt = Si è verificato un errore nell'apertura del tuo database

unused_better-bibtex_db_corrupt_explanation = Si è verificato un errore nell'apertura del tuo database. Questo può indicare che il database è corrotto. L'errore riportato da Zotero è: { $error }.
    
    Si prega di scegliere come si desidera procedere:
    
    1. chiedere a Zotero di recuperare un backup  del database e riavviare
    2. effettuare il reset del database di Better BibTeX database e continuare
    3. chiudere Zotero e contattare il supporto per provare a recuperare il database.
    
    Il reset ti consentirà ti proseguire, ma le chiavi di citazione verranno generate di nuovo e le esportazioni automatiche andranno re-impostate. In questo caso verrà mantenuta una copia di backup del database corrotto.

unused_better-bibtex_db_corrupt_quit = Esci da Zotero
unused_better-bibtex_db_corrupt_reset = Reset e procedi
unused_better-bibtex_db_corrupt_restore = Riavvia Zotero e tenta il ripristino

unused_better-bibtex_error-report_out-of-memory = È probabile che il tuo sistema esaurisca la memoria nel tentativo di costruire il file di log. Si prega di impostare la voce '{ $preference }' a un valore più basso dell'attuale { $limit }, riavviare, quindi provare di nuovo.
unused_better-bibtex_key-manager_regenerate = (ri)generazione delle chiavi di citazione in corso
unused_better-bibtex_key-manager_regenerate_confirm = (ri)generazione di  { $n } chiavi, confermi?
unused_better-bibtex_preferences_advanced = Avanzate
unused_better-bibtex_preferences_advanced_export_workers = Esportazioni parallele in background:
unused_better-bibtex_preferences_advanced_import_unabbreviate = Espandi i titoli di rivista abbreviati durante l'importazione usando la lista:
unused_better-bibtex_preferences_advanced_import_unabbreviate_off = Nessuna
unused_better-bibtex_preferences_advanced_tab_citekeys = Chiavi di citazione
unused_better-bibtex_preferences_advanced_tab_export = Esportazione

unused_better-bibtex_preferences_advanced_warning = Le impostazioni qui presenti possono avere un effetto sulle prestazioni, e causare l'interruzione delle esportazioni. Procedere con cautela.
unused_better-bibtex_preferences_auto-export_collection = Collezione
unused_better-bibtex_preferences_auto-export_library = Biblioteca
unused_better-bibtex_preferences_auto-export_warn_long-running = Avvisami quando le esportazioni automatiche impiegano più di (seconds)
unused_better-bibtex_preferences_citekey_aux-scanner = Analizzatore di file BibTeX AUX/Markdown
unused_better-bibtex_preferences_citekey_citekeys = Chiavi di citazione
unused_better-bibtex_startup_db-upgrade = Pulizia database { $n }/{ $total }...
unused_better-bibtex_startup_db-upgrade_not-editable = { $n } elementi non aggiornati poiché in sola lettura
unused_better-bibtex_startup_db-upgrade_saving = Pulizia database, salvataggio...
unused_better-bibtex_startup_installing_translators_new = Better BibTeX: installati nuovi programmi traduttori
unused_better-bibtex_startup_installing_translators_new_dont-ask-again = Non chiedere di nuovo

unused_better-bibtex_startup_installing_translators_new_drag-and-drop = Sono stati installati nuovi programmi traduttori per Better BibTeX. Se si desidera usarli nelle operazioni di trascinamento (per trascinare citazioni), è necessario riavviare l'applicazione per attivarli.
unused_better-bibtex_startup_loading-keys = Caricamento chiavi di citazione
unused_better-bibtex_startup_loading_databases = Caricamento database
unused_better-bibtex_startup_waiting_for_translators = In attesa dei programmi traduttori di Zotero
unused_better-bibtex_texstudio_not_found = TeXstudio non trovato all'indirizzo { $path }
unused_better-bibtex_zotero-pane_server_disabled = Esportazione via HTTP disabilitata
unused_preferences_auto-export_status_running = in corso
unused_preferences_auto-export_status_scheduled = pianificata

unused_zotero_better-bibtex_csquotes = Sostituisci virgolette doppie o uncinate con \\enquote{"{"}...{"}"} (richiede pacchetto csquotes)
xpi = 
  .description = Rendi Zotero più utile per chi, come noi, è rimasto fedele a LaTeX.
