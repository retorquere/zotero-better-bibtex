...
better-bibtex_error-report_review = Les données ci-dessous sont sur le point d'être soumises à Better BibTeX à fins de débogage. Vous pouvez les examiner pour contrôler s'il y a des informations sensibles que vous ne souhaitez pas envoyer.
...
better-bibtex_preferences_advanced_import_case-protection_warning = À l'importation, BBT protègera la casse (<span class="nocase">...<span>) des mots entre {"{"}Braces{"}"} dans les titres.
...
better-bibtex_preferences_export_fields_language_langid = identifiant de langue
better-bibtex_preferences_export_fields_language_language = langue
...
unused_better-bibtex_db_corrupt_explanation = Une erreur s''est produite lors de l''ouverture de votre base de données. Cela pourrait signifier que votre base de données est corrompue. L''erreur signalée par Zotero est la suivante: { $error }.
