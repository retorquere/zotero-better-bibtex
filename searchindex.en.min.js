var relearn_searchindex=[{breadcrumb:"Better BibTeX for Zotero",content:`Install by downloading the latest release – if you use Firefox, make sure to right-click and save the XPI file, not just clicking it – and then in Zotero:
In the main menu go to Tools > Plugins Select ‘Plugins’ Click on the gear in the top-right corner and choose ‘Install Plugin From File…’ Choose .xpi that you’ve just downloaded, click ‘Install’ After the initial installation, the plugin will auto-update to newer releases, so you should need to perform the process described here only once.
...
Note: BBT needs to be installed in Zotero, not Firefox. There used to be two versions of Zotero, one standalone, and one that was installed in Firefox. BBT is a Zotero extension and lives where Zotero lives; Zotero is no longer available as a Firefox plugin, only as standalone. If you download BBT using Firefox by simply (left)clicking on it, Firefox will download it, think it is a Firefox plugin, try to install it, and will fail with a complaint it is corrupt, hence the right-click and save.`,description:`Install by downloading the latest release – if you use Firefox, make sure to right-click and save the XPI file, not just clicking it – and then in Zotero:
In the main menu go to Tools > Plugins Select ‘Plugins’ Click on the gear in the top-right corner and choose ‘Install Plugin From File…’ Choose .xpi that you’ve just downloaded, click ‘Install’ After the initial installation, the plugin will auto-update to newer releases, so you should need to perform the process described here only once.`,tags:[],title:"Installation",uri:"/zotero-better-bibtex/installation/index.html"},{breadcrumb:"Better BibTeX for Zotero",content:`The Better BibTeX Configuration can be found under the regular Zotero preferences pane, tab ‘Better BibTeX’.
...
Set the pattern used to generate citation keys. The format of the keys is documented [here]({{ ref . “citing” }}). Changing this setting does not affect existign keys - for this you would need to select the items and refresh the keys.
...
Regenerate citation key when item changes default: no
When true, BBT will overwrite existing keys with new keys after an item changes.
Automatically fill citation key after default: 2
...
Options:
across all libraries within each library Ideographs in citekeys Apply kuroshiro romajization in Japanese names/titles. Uses a lot of memory. default: no
...
For those who are curious about what the Clear/Generate BibTeX key right-click options do, this will warn you if you are doing this on more than 10 (default) at the same time, to prevent your curiosity from changing all your citation keys at once.`,description:`The Better BibTeX Configuration can be found under the regular Zotero preferences pane, tab ‘Better BibTeX’.
The configuration of Better BibTeX is a little baroque compared to the standard Zotero Bib(La)TeX exporters (which only have hidden preferences). The defaults should just work, but here’s an attempt to describe what they do.
Making any change here will drop your entire export cache. This is usually not a problem unless you have a really large library, but you can read about what is involved here.`,tags:[],title:"Preferences",uri:"/zotero-better-bibtex/preferences/index.html"},{breadcrumb:"Better BibTeX for Zotero > Preferences",content:`The Better BibTeX Configuration can be found under the regular Zotero preferences pane, tab ‘Better BibTeX’.
The configuration of Better BibTeX is a little baroque compared to the standard Zotero Bib(La)TeX exporters (which only have hidden preferences). The defaults should just work, but here’s an attempt to describe what they do.
Making any change here will drop your entire export cache. This is usually not a problem unless you have a really large library, but you can read about what is involved here.
...
add use-prefix when family name has a prefix default: yes
...
Journal abbreviation mode default: always use the Zotero abbreviation field
...
Options:
always use the Zotero abbreviation field always use the auto-abbreviated value use Zotero abbreviation field, fallback to auto-abbreviation BibTeX/BibLaTeX Export language as default: langid
...
Used for drag-and-drop/quick copy using Better BibTeX citation keys. In the Zotero “Export” pane, choose Better BibTeX Quick Copy as the default export format for quick copy, and choose the desired format for the drag-and-drop citations here. In the case of Eta templates, the selected items are available as it.items. &lt;%= JSON.stringify(it.items) %&gt; will show you the available data on the items.
Options:
LaTeX citation Citation Keys Eta template GitBook org-ref citation org-ref v3 citation Org-mode citation link Org-mode select link Pandoc citation Roam Citation Key RTF Scan marker Zotero select link Jupyter notebook Jekyll LaTeX command default: cite
...
postscript Miscellaneous Sort TeX/CSL output (useful if you use version control on the output): default: citation key (slower on very large libraries)
...
Apply title-casing to titles default: <not set>
...
Cache Enable caching for background exports default: yes
Even though BBT exports happen in a separate thread, some work needs to be done before the background export can start. Part of this work is preloading the cache. You can shorten the (blocking) preparation time by turning off the cache, at the cost of longer export times.`,description:`The Better BibTeX Configuration can be found under the regular Zotero preferences pane, tab ‘Better BibTeX’.
The configuration of Better BibTeX is a little baroque compared to the standard Zotero Bib(La)TeX exporters (which only have hidden preferences). The defaults should just work, but here’s an attempt to describe what they do.
Making any change here will drop your entire export cache. This is usually not a problem unless you have a really large library, but you can read about what is involved here.`,tags:[],title:"Export",uri:"/zotero-better-bibtex/preferences/export/index.html"},{breadcrumb:"Better BibTeX for Zotero",content:`Better BibTeX adds a couple of export formats to Zotero’s export dialog and several ways to improve export for plaintext-based authoring. The Better BibTeX configuration pane can be found under the regular Zotero preferences pane, tab ‘Better BibTeX’ where you can tweak the exports, such as
...
please do report them, and you can (temporarily I hope) disable by going into the BBT advanced preferences and drag the parallel exports slider to 0. If BBT detects an error during background exports, it will disabled them, so that subsequent exports will be foreground exports. You can re-enable them using the same slider by dragging it back to the default 1.`,description:`Better BibTeX adds a couple of export formats to Zotero’s export dialog and several ways to improve export for plaintext-based authoring. The Better BibTeX configuration pane can be found under the regular Zotero preferences pane, tab ‘Better BibTeX’ where you can tweak the exports, such as
Omitting fields from exports to slim down your bib files. Configuring how citation keys are generated. And really a ton more. Additionally you can automate exporting (parts of) your library either using auto-export or pull export.`,tags:["Export","Scripting"],title:"Exporting items",uri:"/zotero-better-bibtex/exporting/index.html"},{breadcrumb:"Better BibTeX for Zotero > Preferences",content:`The Better BibTeX Configuration can be found under the regular Zotero preferences pane, tab ‘Better BibTeX’.
The configuration of Better BibTeX is a little baroque compared to the standard Zotero Bib(La)TeX exporters (which only have hidden preferences). The defaults should just work, but here’s an attempt to describe what they do.
Making any change here will drop your entire export cache. This is usually not a problem unless you have a really large library, but you can read about what is involved here.
...
If you have auto-exports set up, BBT will wait this many seconds before actually kicking off the exports to buffer multiple changes in quick succession setting off an unreasonable number of auto-exports. Minimum is 1 second. Changes to this preference take effect after restarting Zotero.`,description:`The Better BibTeX Configuration can be found under the regular Zotero preferences pane, tab ‘Better BibTeX’.
The configuration of Better BibTeX is a little baroque compared to the standard Zotero Bib(La)TeX exporters (which only have hidden preferences). The defaults should just work, but here’s an attempt to describe what they do.
Making any change here will drop your entire export cache. This is usually not a problem unless you have a really large library, but you can read about what is involved here.`,tags:[],title:"Automatic export",uri:"/zotero-better-bibtex/preferences/automatic-export/index.html"},{breadcrumb:"Better BibTeX for Zotero",content:`Generating citekeys for your items The BibTeX citations keys generated by the standard Zotero exporters are always generated at time of export, using an algorithm that usually generates unique keys. For serious LaTeX users, “usually” presents the following problems:
...
Drag and drop LaTeX citations using these keys to your favorite LaTeX editor Show your citation keys in the item list view. Drag and drop/hotkey citations You can drag and drop citations into your LaTeX/Markdown/Orgmode editor, and it will add a proper \\cite{citekey}/[@citekey]/[[zotero://select...][@citekey]. The cite command is configurable for LaTeX by setting the config option in the preferences. Do not include the leading backslash.
...
(auth + title).len + year Generating citekeys To generate your citekeys, you use a formula composed of functions and filters. Broadly, functions grab text from your item, and filters transform that text.
...
If you are used to the bracketed formula syntax, the syntax has changed to a javascript-ish format. The old syntax was getting harder to maintain and its inflexibility prevented new extensions to the functions being implemented cleanly.`,description:`Generating citekeys for your items The BibTeX citations keys generated by the standard Zotero exporters are always generated at time of export, using an algorithm that usually generates unique keys. For serious LaTeX users, “usually” presents the following problems:
If a non-unique key is generated, which one gets postfixed with a distinguishing character is essentially non-deterministic. The keys are always auto-generated, so if you correct a typo in the author name or title, the key will change You can’t see the citation keys until you export them For a LaTeX author, the citation keys have their own meaning, fully separate from the other entry data, even if people usually pick a naming scheme related to them. As the citation key is the piece of data that connects your bibliography, this is a piece of data you want to have control over. BBT offers you this control:`,tags:["Citation Keys"],title:"Citation Keys",uri:"/zotero-better-bibtex/citing/index.html"},{breadcrumb:"Better BibTeX for Zotero > Exporting",content:`To export a library, group or collection, right-click on it in the left Zotero pane and choose “Export Library…” or “Export Collection…”. With BBT’s export translators (e.g., “Better BibTeX”), checking the Keep updated option will register the export for automation. After you’ve completed the current export, any changes to the collection or library will trigger an automatic re-export to update the file. You can review/remove exports from the BBT preferences. While I’ve gone to some lengths to make sure performance is OK, don’t go overboard with the number of auto-exports you have going. Also, exporting only targeted selections over your whole library will get you better performance. You can set up separate exports for separate papers for example if you have set up a collection for each.
...
Auto-export + cloud-sync + import by URL pro: gets the full Better BibTeX / Better BibLaTeX file into Overleaf pro: free or already included in what you pay for your cloud service con: requires cloud service which has direct-download links (Dropbox seems to offer this, as does Onedrive, but I haven’t tested this) Auto-export + Dropbox - Overleaf integration pro: gets the full Better BibTeX / Better BibLaTeX file into Overleaf con: paid option on Overleaf Auto-export + git: pro: gets the full BibTeX file to Overleaf, tested and works con: requires technical expertise to set up, paid option on overleaf DropBox / Google Drive / Box.com / … Or any other of the plethora of choices that are available today that will “cloud-sync” your files. Will work with any cloud service, as long as it delivers direct-download links, as this method will import into Overleaf by URL. 1.Export the Better BibTeX / Better BibLaTeX file into an cloud shared folder, with the “keep updated”-option checked 2.Create/generate/find a direct-download link. Google is your friend. 3.Import your cloud-saved bib-file into Overleaf by “Add file -> From external URL” 4.When adding new sources, refresh both the Zotero client, and click the “refresh”-button in the file on Overleaf
...
the response body will be [#adams2001][][#brigge2002][], and [#adams2001][][#brigge2002][] will be left on the clipboard For Juris-M, the port number 23119 must be replaced with 24119. ↩︎`,description:`PSA: as of Zotero 5.0.71, access to the CAYW URL will no longer work from the browser for security reasons; curl and other programmatic access such as from editors access will work.
Good news for TeXnicians and those down with Mark (aka Markdown, RST, whatnot): this is the time to go pester the author of your favorite editor for Zotero integration!
Editor integration vim Graciously supplied by David Lukes:`,tags:["Citation Keys","Overleaf","Cite as You Write"],title:"Cite as you Write",uri:"/zotero-better-bibtex/citing/cayw/index.html"},{breadcrumb:"Better BibTeX for Zotero > Preferences",content:`The Better BibTeX Configuration can be found under the regular Zotero preferences pane, tab ‘Better BibTeX’.
The configuration of Better BibTeX is a little baroque compared to the standard Zotero Bib(La)TeX exporters (which only have hidden preferences). The defaults should just work, but here’s an attempt to describe what they do.
Making any change here will drop your entire export cache. This is usually not a problem unless you have a really large library, but you can read about what is involved here.
...
Sentence-case titles on import: Sentence-case titles on import: default: yes, but try to exclude already-sentence-cased titles
...
Options:
yes, but try to exclude already-sentence-cased titles yes no (import titles as-is) Migrate BetterBibTeX preferences/citation keys`,description:`The Better BibTeX Configuration can be found under the regular Zotero preferences pane, tab ‘Better BibTeX’.
The configuration of Better BibTeX is a little baroque compared to the standard Zotero Bib(La)TeX exporters (which only have hidden preferences). The defaults should just work, but here’s an attempt to describe what they do.
Making any change here will drop your entire export cache. This is usually not a problem unless you have a really large library, but you can read about what is involved here.`,tags:[],title:"Import",uri:"/zotero-better-bibtex/preferences/import/index.html"},{breadcrumb:"Better BibTeX for Zotero",content:`Before all else, thank you for taking the time for submitting an issue, and I’m sorry that I’ve probably interrupted your flow.
...
<i>...</i>⇔\\emph{...}/\\mkbibemph{...}/\\textit{...} <b>...</b>⇔\\textbf{...} <sup>...</sup>⇔\\textsuperscript{...} and <sub>...</sub>⇔\\textsubscript{...}. More can be added on request.
...
if (Translator.BetterTeX) { tex.add({ name: 'language', value: tex.langid() }) }`,description:"Add-on could not be installed because it appears to be corrupt. You have downloaded the Better BibTeX plugin by clicking on the download link using Firefox. As Zotero and Firefox use the same plugin technology, Firefox think the BetterBibTeX plugin is intended for itself, tries to install it, and then finds out it won’t work. You need to download the plugin without installinging it into Firefox by right-clicking the download link, choose save-as, and then install it into Zotero using installation instructions to get started with BBT.",tags:["Export"],title:"Frequently Asked Questions",uri:"/zotero-better-bibtex/support/faq/index.html"},{breadcrumb:"Better BibTeX for Zotero > Exporting",content:`In addition to LaTeX, BBT plays very well with pandoc:
...
With regards to sorting citations within a cluster, this is how Zotero does it by default, where pandoc keeps citations in the order you entered them. You can override this by setting sorted to false, for example to retain a prefix at the front, but it may generated citations that are not style-compliant.`,description:`In addition to LaTeX, BBT plays very well with pandoc:
you can drag and drop citations from Zotero into your markdown documents. you can cite as you write in your favorite editor with varying levels of comfort, mostly depending on how easy (VSCode, Sublime) or hard (looking at you Scrivener) it is to extend your editor. you can even convert your markdown document into a LibreOffice/Word document with actual live Zotero items as if you had entered them into Zotero all along (see below) Use CSL, not bibtex with pandoc Many tutorials on the use of pandoc to generate documents with citations seem to use bibtex as a bibliography format. I would encourage the use of CSL instead. Internally, both Zotero and pandoc-citeproc use CSL citation engines; the two options you have are:`,tags:[],title:"Markdown/Pandoc",uri:"/zotero-better-bibtex/exporting/pandoc/index.html"},{breadcrumb:"Better BibTeX for Zotero > Citation Keys",content:`So you have decided that enough is enough, and you want to migrate your existing Word/Libreoffice document to LaTeX/Markdown. Plenty (well…) tools exist to help with the migration of your document content, pandoc being the most prominent one, but one thing none of them will do is keep your citations intact. This will not do.
...
pandoc -f docx+citations -t markdown -i Aristotle.docx -o Aristotle.md should do the trick!`,description:`So you have decided that enough is enough, and you want to migrate your existing Word/Libreoffice document to LaTeX/Markdown. Plenty (well…) tools exist to help with the migration of your document content, pandoc being the most prominent one, but one thing none of them will do is keep your citations intact. This will not do.
If you install this CSL style in Zotero (or modify it further and use that), Zotero will render the in-text citations as [@citekey] when you ask Zotero to render the bibliography. If you want LaTeX, modify the style accordingly. You can then put your document through pandoc or whatnot to get LaTeX/Markdown.`,tags:["Citation Keys","Word","Libreoffice"],title:"Migrating from Word/Libreoffice",uri:"/zotero-better-bibtex/citing/migrating/index.html"},{breadcrumb:"Better BibTeX for Zotero > Preferences",content:`The Better BibTeX Configuration can be found under the regular Zotero preferences pane, tab ‘Better BibTeX’.
The configuration of Better BibTeX is a little baroque compared to the standard Zotero Bib(La)TeX exporters (which only have hidden preferences). The defaults should just work, but here’s an attempt to describe what they do.
Making any change here will drop your entire export cache. This is usually not a problem unless you have a really large library, but you can read about what is involved here.
Fields Treat the extra field as a note default: no
Treat the Zotero extra field as a note.
When merging items, also merge: their citation keys into an bib(la)tex ids field default: no
...
Options:
No Assume single-word fields to be @string vars Match against the @string declarations below Match against the @string declarations and their values below Performance measurement`,description:`The Better BibTeX Configuration can be found under the regular Zotero preferences pane, tab ‘Better BibTeX’.
The configuration of Better BibTeX is a little baroque compared to the standard Zotero Bib(La)TeX exporters (which only have hidden preferences). The defaults should just work, but here’s an attempt to describe what they do.
Making any change here will drop your entire export cache. This is usually not a problem unless you have a really large library, but you can read about what is involved here.`,tags:[],title:"Miscellaneous",uri:"/zotero-better-bibtex/preferences/miscellaneous/index.html"},{breadcrumb:"Better BibTeX for Zotero > Exporting",content:`You can fetch your bibliography on the url http://127.0.0.1:23119/better-bibtex/collection?[collectionID].[format] 1. You can get this URL for a group, library or collection by right-clicking it and selecting Download Better BibTeX export...
...
Replace portnumber 23119 with 24119 for Juris-M. ↩︎`,description:`You can fetch your bibliography on the url http://127.0.0.1:23119/better-bibtex/collection?[collectionID].[format] 1. You can get this URL for a group, library or collection by right-clicking it and selecting Download Better BibTeX export...
You can add options to the export as URL parameters:
&exportNotes=[true|false] &useJournalAbbreviation=[true|false] You can fetch your library as part of your build, using something like curl from your Makefile, or with a BibLaTeX remote statement like
\\addbibresource[location=remote]{http://127.0.0.1:23119/better-bibtex/collection?/0/8CV58ZVD.biblatex} format can be:`,tags:["Export"],title:"Pull export",uri:"/zotero-better-bibtex/exporting/pull/index.html"},{breadcrumb:"Better BibTeX for Zotero > Preferences",content:`The Better BibTeX Configuration can be found under the regular Zotero preferences pane, tab ‘Better BibTeX’.
The configuration of Better BibTeX is a little baroque compared to the standard Zotero Bib(La)TeX exporters (which only have hidden preferences). The defaults should just work, but here’s an attempt to describe what they do.
Making any change here will drop your entire export cache. This is usually not a problem unless you have a really large library, but you can read about what is involved here.
...
importPlaceEvent default: inproceedings,conference,presentation,talk
...
Edit the hidden preference extensions.zotero.translators.better-bibtex.postscriptOverride, and set it to a filename like postscript.js 2. In the directory where you intend to export to, create a file called postscript.js (or whatever you set the preference to) and add the postscript you want there 3. Export to that directory. A postscript override will disable caching for that export. preferencesOverride default: <not set>
You can use custom preferences per export directory:
Edit the hidden preference extensions.zotero.translators.better-bibtex.preferencesOverride, and set it to a filename like preferences.json In the directory where you intend to export to, create a file called preferences.json (or whatever you set the preference to), or called [bibfile you are exporting to].json and add the desired preference overrides in the format {"override": { "preferences": {"skipFields": "note"} } }. You can get your current preferences by exporting to BetterBibTeX JSON and removing everything except config.preferences, and renaming config to override. Export to that directory. A preferences override will disable caching for that export.
...
Edit the hidden preference extensions.zotero.translators.better-bibtex.stringstOverride, and set it to a filename like strings.bib In the directory where you intend to export to, create a file called strings.bib (or whatever you set the preference to) and add the @string declarations you want there Export to that directory. A strings override will disable caching for that export. verbatimFields default: url,doi,file,pdf,ids,eprint,/^verb[a-z]$/,groups,/^citeulike-linkout-[0-9]+$/,/^bdsk-url-[0-9]+$/, keywords
...
Both Zotero and BBT expect titles to be in sentence-case, but a lot of sites offer import data that is Title Cased. When exporting these titles to bib(la)tex you’re going to get a lot of extra unwanted braces, because all these Title Cased words will look like proper nouns to BBTs own title-casing mechanism. When this setting is on, you will be warned when you import/save items in Zotero with titles that look like they’re Title Cased, so that you can inspect/correct them.`,description:`The Better BibTeX Configuration can be found under the regular Zotero preferences pane, tab ‘Better BibTeX’.
The configuration of Better BibTeX is a little baroque compared to the standard Zotero Bib(La)TeX exporters (which only have hidden preferences). The defaults should just work, but here’s an attempt to describe what they do.
Making any change here will drop your entire export cache. This is usually not a problem unless you have a really large library, but you can read about what is involved here.`,tags:[],title:"Hidden preferences",uri:"/zotero-better-bibtex/preferences/hidden-preferences/index.html"},{breadcrumb:"Better BibTeX for Zotero > Support",content:`The Better BibTeX exporters are a lot slower than the standard Zotero Bib(La)TeX exporters. If you have a small library, you will not likely notice this, but if you have several thousand items, and you are in the habit of exporting substantial parts of your library, this gets annoying really fast.
...
The same applies to upgrades. As the export behavior quite frequently changes between versions, Better BibTeX will drop the cache during first startup of the newer version. This can be a nuisance if you have a large library, but it’s the only way to keep support feasible.`,description:`The Better BibTeX exporters are a lot slower than the standard Zotero Bib(La)TeX exporters. If you have a small library, you will not likely notice this, but if you have several thousand items, and you are in the habit of exporting substantial parts of your library, this gets annoying really fast.
To deal with this problem, Better BibTeX implements an extensive caching system. With a filled cache, Better BibTeX is substantially faster than the default Zotero exporters. Specifically for automatic background exports, a filled cache is a good thing to have.`,tags:["Export","Cache"],title:"Performance",uri:"/zotero-better-bibtex/support/performance/index.html"},{breadcrumb:"Better BibTeX for Zotero > Citation Keys",content:`You can populate a collection, or tag items in your library, from an existing paper by scanning the aux file generated by bibtex, or a pandoc-markdown file, for referenced used. Scanning can be triggered from the Tools menu (for tagging the cited items) or by right-clicking a collection (for adding them to a collection). The scanner will read your AUX files and will put entries you cited in the associated LaTeX document into the current collection.
...
name: name of the bib(la)tex field to output value: the value for the field without LaTeX encoding bibtex: the value for the field with LaTeX encoding already applied. If both bibtex and value are present, bibtex takes precedence enc: specifies how to encode the value field. Valid values are: latex: encode markup and special characters to LaTeX. This is the default, if you don’t provide an enc parameter, latex is assumed verbatim: encode under verbatim rules literal: encode under literal rules raw: assume value is already LaTeX-encoded (same as passing the value in bibtex) url: encode as verbatim url sep: if value is an array, and enc is latex, encode each array element using latex and join the results with the string in sep. Defaults to an empty string. html: boolean indicating whether the value is full HTML (really only useful for notes) exportCaseProtection: boolean indicating whether export case-protection processing is applied. e.g. change the value of year in output tex.add({name: 'year', value: "your_year_value"})
...
if (Translator.BetterTeX && !Translator.options.exportFileData && zotero.attachments && zotero.attachments.length) { for (const att of zotero.attachments) { if (att.localPath) { att.localPath = att.localPath.replace(/^\\/.*?\\/.*?\\//, '~/') } } tex.add({ name: 'file', value: zotero.attachments, enc: 'attachments' }) return { cache: false } } From discussion here.
...
if (Translator.BetterCSLJSON) { csl.file = zotero.attachments.map(a => a.localPath).join(";"); } Convert Windows attachment paths to Unix if (Translator.BetterTeX && !Translator.options.exportFileData && zotero.attachments && Translator.exportPath.includes('\\\\\\\\')) { if (zotero.attachments) { tex.add({ name: 'file', bibtex: tex.enc_attachments({ value: zotero.attachments }, path => path.replace(/^[A-Z]:/i, '').replace(/\\\\\\\\/g, '/')) }) } }`,description:"You wanted customized… You got customized. If you go into the Export tab of the Better BibTeX preferences, subtab postscript, you will find a text box (empty by default) where you can edit a javascript snippet which will be executed for each entry generated in the Bib(La)TeX exporter. In this code, you have access to the entry just before it will be written out and cached. There is an API to do this, and it’s fairly stable, but usually you can just open a new issue and ask me to write it, and I’ll add it here (it’s how the examples got here). Postscripts are available in 4 of the translators:",tags:["Scripting"],title:"Scripting",uri:"/zotero-better-bibtex/exporting/scripting/index.html"},{breadcrumb:"Better BibTeX for Zotero > Support",content:"BBT is currently available in English, German, French and Brazilian Portuguese. You can help out by adding new translations or fixing existing ones on Crowdin.",description:"BBT is currently available in English, German, French and Brazilian Portuguese. You can help out by adding new translations or fixing existing ones on Crowdin.",tags:[],title:"Translations",uri:"/zotero-better-bibtex/support/translations/index.html"},{breadcrumb:"Better BibTeX for Zotero > Exporting",content:`In some cases the Zotero fields do not offer a place to enter the data you need to get your exported entries just right. For this Zotero has a so-called “cheater syntax” which allows you to add extra “fields” as separate lines in the extra field all items in Zotero have. These fields are supported by the citation processor inside Zotero, and BBT understands them too, and adds one “cheater syntax of its own.
...
label type zotero csl access date text accessDate accessed accessed date accessDate accessed annote text annote application number text applicationNumber call-number archive text archive archive archive collection text archive_collection archive id text archiveID / number archive location text archiveLocation archive_location archive place text archive-place artist name artist artwork medium text artworkMedium / medium artwork size text artworkSize dimensions assignee text assignee attorney agent name attorneyAgent audio file type text audioFileType / medium audio recording format text audioRecordingFormat / medium author name author / creator author authority text authority authority available date date available-date bill number text billNumber / number blog title text blogTitle / publicationTitle book author name bookAuthor container-author book title text bookTitle / publicationTitle call number text applicationNumber / callNumber call-number cartographer name cartographer case name text caseName / title cast member name castMember performer categories text categories chair name chair chair chapter number text session chapter-number citation key text citationKey citation-key citation label text citation-label citation number text citation-number code text code container-title code number text codeNumber volume code pages text codePages / pages code volume text codeVolume / volume collection editor name seriesEditor collection-editor collection number text seriesNumber collection-number collection title text series / seriesTitle collection-title commenter name commenter committee text committee section company text company / publisher compiler name compiler composer name composer composer conference name text conferenceName event-title container author name bookAuthor container-author container title text code / publicationTitle / reporter container-title container title short text container-title-short contributor name contributor contributor cosponsor name cosponsor counsel name counsel country text country court text authority / court creator name creator author curator name curator custom custom date date date issued date decided date date / dateDecided date enacted date date / dateEnacted dictionary title text dictionaryTitle / publicationTitle dimensions text artworkSize / runningTime dimensions director name director director distributor text distributor / publisher division text division docket number text docketNumber / number document number text documentNumber / number doi text DOI DOI edition text edition edition editor name editor editor editorial director name editorial-director encyclopedia title text encyclopediaTitle / publicationTitle episode number text episodeNumber / number event text event event date date event-date event place text eventPlace event-place event title text conferenceName / meetingName event-title executive producer name executiveProducer executive-producer filing date date filingDate submitted first page text firstPage / pages first reference note number text first-reference-note-number format text format / medium forum title text forumTitle / publicationTitle genre text genre / programmingLanguage / type genre guest name guest guest history text history references host name host / podcaster host id text id identifier text identifier / number illustrator name illustrator institution text institution / publisher interviewee name interviewee interviewer name interviewer interviewer interview medium text interviewMedium / medium inventor name inventor isbn text ISBN ISBN issn text ISSN ISSN issue text issue / priorityNumbers issue issued date date issued issue date date date / issueDate issuing authority text authority / issuingAuthority journal abbreviation text journalAbbreviation journalAbbreviation jurisdiction text jurisdiction keyword text keyword label text label / publisher language text language language legal status text legalStatus / status legislative body text authority / legislativeBody letter type text letterType / type library catalog text libraryCatalog source license text rights license locator text locator manuscript type text manuscriptType / type map type text mapType / type medium text medium / system medium meeting name text meetingName event-title name of act text nameOfAct / title narrator name narrator narrator network text network / publisher number text number number number of pages text numPages number-of-pages number of volumes text numberOfVolumes number-of-volumes num pages text numPages number-of-pages organization text authority / organization organizer name organizer organizer original author name originalCreator original-author original creator name originalCreator original-author original date date originalDate original-date original place text originalPlace original-publisher-place original publisher text originalPublisher original-publisher original publisher place text originalPlace original-publisher-place original title text original-title page text pages page page first text page-first pages text pages page part text part part number text partNumber part-number part title text partTitle part-title patent number text number / patentNumber performer name castMember / performer performer place text place publisher-place pmcid text PMCID PMCID pmid text PMID PMID podcaster name podcaster host post type text postType / type presentation type text presentationType / type presenter name presenter printing text printing priority date date originalDate / priorityDate priority numbers text priorityNumbers issue proceedings title text proceedingsTitle / publicationTitle producer name producer producer programmer name programmer programming language text programmingLanguage genre program title text programTitle / publicationTitle publication title text publicationTitle container-title public law number text number / publicLawNumber publisher text publisher publisher publisher place text place publisher-place recipient name recipient recipient references text history / references references reporter text reporter container-title reporter volume text reporterVolume / volume report number text number / reportNumber report type text reportType / type repository text publisher / repository repository location text place / repositoryLocation reviewed author name reviewedAuthor reviewed-author reviewed genre text reviewed-genre reviewed title text reviewed-title rights text rights license running time text runningTime dimensions scale text scale scale scriptwriter name scriptwriter script-writer script writer name scriptwriter script-writer section text committee / section section series text series collection-title series creator name seriesCreator series-creator series editor name seriesEditor collection-editor series number text seriesNumber collection-number series text text seriesText series title text seriesTitle collection-title session text session chapter-number session title text publicationTitle / sessionTitle short title text shortTitle shortTitle / title-short source text libraryCatalog source sponsor name sponsor status text status status studio text publisher / studio subject text subject / title submitted date filingDate submitted supplement text supplement system text system medium thesis type text thesisType / type title text title title title short text shortTitle title-short translator name translator translator type text type genre / type university text publisher / university url text url URL version text versionNumber version version number text versionNumber version video recording format text medium / videoRecordingFormat volume text codeNumber / volume volume volume title text volume-title volume title short text volume-title-short website title text publicationTitle / websiteTitle website type text type / websiteType words by name wordsBy year suffix text year-suffix ¹ only supported in Juris-M`,description:"In some cases the Zotero fields do not offer a place to enter the data you need to get your exported entries just right. For this Zotero has a so-called “cheater syntax” which allows you to add extra “fields” as separate lines in the extra field all items in Zotero have. These fields are supported by the citation processor inside Zotero, and BBT understands them too, and adds one “cheater syntax of its own.",tags:[],title:"Extra fields",uri:"/zotero-better-bibtex/exporting/extra-fields/index.html"},{breadcrumb:"Better BibTeX for Zotero > Support",content:`Please DO NOT DO THIS without an open issue on the github tracker. I’ve been running tests this way for years, but I’d still prefer to guide you through this to prevent any risks to your citation library. The test suite has only been tested on MacOS and Linux. I doubt it’ll run on Windows, and I don’t know what will go wrong when you test. If you’re on Windows, DO NOT run this on the same account that holds your actual Zotero library.
...
rm ~/Zotero mv ~/Zotero.saved ~/Zotero and you can remove the copy of the profile administration you made earlier. Zotero should now start again normally opening your library.`,description:"Please DO NOT DO THIS without an open issue on the github tracker. I’ve been running tests this way for years, but I’d still prefer to guide you through this to prevent any risks to your citation library. The test suite has only been tested on MacOS and Linux. I doubt it’ll run on Windows, and I don’t know what will go wrong when you test. If you’re on Windows, DO NOT run this on the same account that holds your actual Zotero library.",tags:[],title:"Running the BBT test suite",uri:"/zotero-better-bibtex/support/tests/index.html"},{breadcrumb:"Better BibTeX for Zotero > Exporting",content:`LaTeX en unicode If you’re lucky and you live in the 21st century or later, you can just use unicode in BibLaTeX and you don’t have to bother about anything that follows except if you’re the curious kind.
...
returns: string
citekeys: An array of citekeys format: A specification of how the bibliography should be formatted .quickCopy: Format as specified in the Zotero quick-copy settings .contentType: Output as HTML or text .locale: Locale to use to generate the bibliography .id: CSL style to use Generate a bibliography for the given citekeys
item.citationkey(item_keys: string[] | ‘selected’)
returns: { [string]: string }
item_keys: A list of [libraryID]:[itemKey] strings. If [libraryID] is omitted, assume ‘My Library’ Fetch citationkeys given item keys
item.collections(citekeys: string[], includeParents?: boolean)
returns: { [string]: { key: string; name: string } }
citekeys: An array of citekeys includeParents: Include all parent collections back to the library root Fetch the collections containing a range of citekeys
item.export(citekeys: string[], translator: string, libraryID?: string | number)
returns: string
citekeys: Array of citekeys translator: BBT translator name or GUID libraryID: ID of library to select the items from. When omitted, assume ‘My Library’ Generate an export for a list of citekeys
item.notes(citekeys: string[])
returns: { [string]: { note: string }[] }
citekeys: An array of citekeys Fetch the notes for a range of citekeys
item.pandoc_filter(citekeys: string[], asCSL: boolean, libraryID?: string | number | string[], style?: string, locale?: string)
returns: any
citekeys: Array of citekeys asCSL: Return the items as CSL libraryID: ID of library to select the items from. When omitted, assume ‘My Library’ Generate an export for a list of citekeys, tailored for the pandoc zotero filter
item.regenerate_key(citekeys: string[], library?: string | number)
returns: { [string]: string | null }
citekeys: Array of citekeys whose items should be regenerated. library: The libraryID to search in (optional). Pass * to search across your library and all groups. Regenerate citekeys from current item metadata. For each input citekey, the underlying item’s key is recomputed using the configured citekeyFormat.
Useful when items were created in two phases — initial record from partial metadata, enrichment landing later — and the key generated during phase one no longer matches what current metadata would produce.
...
includeCollections: Wether or not the result should include a list of collection for each library (default is false) List the libraries (also known as groups) the user has in Zotero
viewer.viewPDF(id: string, page: number)
id: id in the form of http://zotero.org/users/12345678/items/ABCDEFG0 page: Page Number, counting from zero Open the PDF associated with an entry with a given id. the id can be retrieve with e.g. item.search(“mypdf”) -> result[0].id
mind that the items.export method had a bug where it would double-wrap the JSON-RPC response; the extra layer has been removed in 6.7.143, but if you were expecting the previous result you will have to update your code.`,description:`You can call into BBT using JSON-RPC on the URL http://localhost:23119/better-bibtex/json-rpc . An example could look like:
curl http://localhost:23119/better-bibtex/json-rpc -X POST -H "Content-Type: application/json" -H "Accept: application/json" --data-binary '{"jsonrpc": "2.0", "method": "collection.scanAUX", "params": ["/My Library/thesis/article1", "/Users/phantom/Downloads/output.aux"] }' The available methods are:
api.ready()
returns: { betterbibtex: string; zotero: string }
Returns the Zotero and BetterBibTeX version to show the JSON-RPC API is ready.
autoexport.add(collection: string, translator: string, path: string, displayOptions?: { [string]: boolean }, replace: boolean=false)`,tags:[],title:"JSON-RPC",uri:"/zotero-better-bibtex/exporting/json-rpc/index.html"},{breadcrumb:"Better BibTeX for Zotero > Installation",content:`Better BibTeX bundles 5 translators you might care about:
...
BetterBibTeX JSON exports and imports items in BetterBibTeX debug format. The error reporter uses this format Better BibTeX Quick Copy exports citations to be copy-pasted into your LaTeX/Markdown document in the form \\cite{< key >}/[@key]`,description:"Zotero import/export translators bundled with BBT",tags:["Export","Import"],title:"Bundled translators",uri:"/zotero-better-bibtex/installation/bundled-translators/index.html"},{breadcrumb:"Better BibTeX for Zotero",content:`v9.0.21 Hidden preference instructions updated Cloned items have their citation keys cleared automatically to prevent duplicates CAYW works again Date parsing logic was enhanced to support better timezone offset detection and date interval handling Collection path resolution support fixed for library and group IDs in absolute paths Pattern formatting for author initials was updated to correctly handle cases where full names are requested instead of just substrings v9.0.10 Citation key at the top v9.0.8 preference autoPinDelay has been renamed to fillKeyAfter preference autoPinOverwrite has been renamed to resetKeyOnChange native Zotero citation key field is now hidden and replaced with a field at the top v9.0.7 localization cleanup updated language support in the language citekey formatting function and langid output improved test reliability by sending only SIGTERM auto-updates are back in the context menu for libraries/groups v9.0.6 fixed raw encoding for tex.X fields auto-export loading fixed removed obsolete clean_pane_persist v9.0.0 removed lokijs which gave unpredictable errors when storing auto-exports new major (should have been done earlier) as BBT is now strictly Zotero 8+ v8.0.40 removed pause-auto-export-during sync. Sync will activate auto-exports and the auto-export debounce will handle the rest Switched key migration to use Sqlite.sys.mjs instead of Zotero.DBConnection. Refactored lokijs object updates re-use objects retrieved from find (spread does not always update) v8.0.38 Context-aware menu visibility: Menu items for citation keys (pin, fill, refresh, copy) now hide automatically when no valid, editable items are selected. Manual “Pin” command: pinning is back for people who have autoPinOverwrite on Citation key pattern update: a new key formula function pinned was added, that’s always prepended Fix: BBT would stall zotero shutdown new translations v8.0.25 fix: read-only groups would error out key migration fix: enable re-migration after start v8.0.22 Re-migration option for citation keys Skip non-editable items Show number of conflicts before migration Migration UI wording improvements v8.0.20 dynamic citation keys are back, option in migration new the pinned function for citation key patterns, which retrieves a pinned citation key from the extra field. autoPinDelay defaults to on (2 seconds) migration uses direct DB read rather than attach/detach, windows too clingy and rename failed v8.0.18 Migration improvements:
prevent accidental locking of BBT database during migration Performance: adds a skipNotifier flag when saving migrated items, avoiding triggering of auto-exports v8.0.17 Updated terminology changing “pin” to “fill” for clarity Reliable renaming of the legacy database after a successful migration. Auto-export storage fixed (would appear to have been deleted after a restart) v8.0.16 Citekey migration skips the update of the ‘Date Modified’ field. Dependencies, including the zotero-plugin-toolkit and zotero-types, have been updated. v8.0.15 A notification flash has been added to inform users when citation key migration has started. v8.0.14 Internal logic for loading items during migration has been improved. Key confict handling removed v8.0.13 Compatibility for Zotero 7.0.32 v8.0.12 The minimum required Zotero version was bumped to 7.0.32. Plugin testing was expanded to include Zotero 7 specifically. v8.0.11 Dependency versions for libraries like ajv and zotero-plugin were clamped to ensure stability. v8.0.10 Enforcement of the minimum Zotero version 7.0.32 requirement was finalized. v8.0.9 UI phrasing was updated in the migration tool to offer a clearer postponement option (“Do not migrate; ask me again at next start”). v8.0.8 A direct link to migration discussions was added to the migration tool to assist users with the transition. v8.0.7 Compatibility status was updated: Zotero 8 and 9 beta are fully supported, while Zotero 7.0.32 is now considered legacy and will not receive ongoing support. The migration dialog now includes an explanation that Better BibTeX citation keys are unavailable until the migration to the native field is complete. v8.0.6 The migration dialog text was further refined for better user understanding. v8.0.5 Additional help text was added to the migration UI for users who are unclear about their choices. v8.0.4 The migration tool now reports the total count of native Zotero citation keys already present. Migration options were clarified to distinguish between moving all keys versus only pinned keys. v8.0.2 The plugin manifest was updated to reflect the new supported Zotero version ranges. v8.0.1 Handling for legacy imports and configuration modules was improved for better reliability. v8.0.0 (Major Release) With the advent of Zotero 8, items have a Zotero-native citation key field. This has replaced the BBT citation key field.
This has caused a few somewhat disruptive changes:
The citation key no longer sits at the top of the item pane. It now sits somewhere in the middle and you may have to scroll to see it. This placement is not under my control Zotero will have moved all pinned keys out of the extra field into the native field The concept of pinning keys is gone; keys are always pinned now. Zotero doesn’t have a place I can store whether a key is pinned or not. The Zotero-native citation keys are stored in another place than the BBT citation keys. If you have no Zotero-native citation keys yet, BBT will silently migrate them to there. If you do have Zotero-native citation keys, and a migration would overwrite them, you will be offered a windows with the choice on how to migrate your citation keys from the BBT storage to the Zotero storage. I have enabled auto-pin (what really is auto-fill now) even you had it turned off. You can still turn it back off if you don’t want this. Upside to all of this is that keys will sync.
Changes Zotero 8 Integration: Items now use a native Zotero citation key field, which replaces the legacy BBT citation key field. Key Pinning Changes: The concept of pinning is technically gone; because Zotero lacks a specific “pinned” toggle, keys are now always pinned. Feature Renaming: The “auto-pin” feature has been effectively renamed to “auto-fill” to align with native Zotero behavior. Bug Fixes: Resolved an issue where the originalDate field was not working with citation keys (#3392) and fixed the retrieval of shortjournal abbreviations (#3382). Database Migration: The internal database for tracking keys and exports has been migrated to LokiJS. Internationalization: Integrated new and updated translations for French, German, Italian, Chinese Simplified, and Brazilian Portuguese. Cleanup: The unused Alchemy.js graph editor was removed from the documentation.`,description:"v9.0.21 Hidden preference instructions updated Cloned items have their citation keys cleared automatically to prevent duplicates CAYW works again Date parsing logic was enhanced to support better timezone offset detection and date interval handling Collection path resolution support fixed for library and group IDs in absolute paths Pattern formatting for author initials was updated to correctly handle cases where full names are requested instead of just substrings v9.0.10 Citation key at the top v9.0.8 preference autoPinDelay has been renamed to fillKeyAfter preference autoPinOverwrite has been renamed to resetKeyOnChange native Zotero citation key field is now hidden and replaced with a field at the top v9.0.7 localization cleanup updated language support in the language citekey formatting function and langid output improved test reliability by sending only SIGTERM auto-updates are back in the context menu for libraries/groups v9.0.6 fixed raw encoding for tex.X fields auto-export loading fixed removed obsolete clean_pane_persist v9.0.0 removed lokijs which gave unpredictable errors when storing auto-exports new major (should have been done earlier) as BBT is now strictly Zotero 8+ v8.0.40 removed pause-auto-export-during sync. Sync will activate auto-exports and the auto-export debounce will handle the rest Switched key migration to use Sqlite.sys.mjs instead of Zotero.DBConnection. Refactored lokijs object updates re-use objects retrieved from find (spread does not always update) v8.0.38 Context-aware menu visibility: Menu items for citation keys (pin, fill, refresh, copy) now hide automatically when no valid, editable items are selected. Manual “Pin” command: pinning is back for people who have autoPinOverwrite on Citation key pattern update: a new key formula function pinned was added, that’s always prepended Fix: BBT would stall zotero shutdown new translations v8.0.25 fix: read-only groups would error out key migration fix: enable re-migration after start v8.0.22 Re-migration option for citation keys Skip non-editable items Show number of conflicts before migration Migration UI wording improvements v8.0.20 dynamic citation keys are back, option in migration new the pinned function for citation key patterns, which retrieves a pinned citation key from the extra field. autoPinDelay defaults to on (2 seconds) migration uses direct DB read rather than attach/detach, windows too clingy and rename failed v8.0.18 Migration improvements:",tags:[],title:"Changelog",uri:"/zotero-better-bibtex/changelog/index.html"},{breadcrumb:"Better BibTeX for Zotero > Tags",content:"",description:"",tags:[],title:"Tag :: Automatic Export",uri:"/zotero-better-bibtex/tags/automatic-export/index.html"},{breadcrumb:"Better BibTeX for Zotero > Tags",content:"",description:"",tags:[],title:"Tag :: Cache",uri:"/zotero-better-bibtex/tags/cache/index.html"},{breadcrumb:"Better BibTeX for Zotero",content:"",description:"",tags:[],title:"Categories",uri:"/zotero-better-bibtex/categories/index.html"},{breadcrumb:"Better BibTeX for Zotero > Tags",content:"",description:"",tags:[],title:"Tag :: Citation Keys",uri:"/zotero-better-bibtex/tags/citation-keys/index.html"},{breadcrumb:"Better BibTeX for Zotero > Tags",content:"",description:"",tags:[],title:"Tag :: Cite as You Write",uri:"/zotero-better-bibtex/tags/cite-as-you-write/index.html"},{breadcrumb:"Better BibTeX for Zotero > Tags",content:"",description:"",tags:[],title:"Tag :: Export",uri:"/zotero-better-bibtex/tags/export/index.html"},{breadcrumb:"Better BibTeX for Zotero > Citation Keys",content:`Note: a number of functions below talk about the author’s lastname; you can read that as “when available”. If you have the name as a single-field name (for entities like International Business Machines or Aristoteles), Zotero doesn’t have a last name, and the full single-field name is taken instead.
Functions auth(n: number=0, m: number=1, creator: '*' | 'author' | 'collaborator' | 'editor' | 'translator'='*', initials: boolean=false)The first n (default: all) characters of the mth (default: first) author's last name.
parameter value assumed when not provided valid values n the number of characters to take from the name, 0 = all 0 number m select the mth author 1 number creator kind of creator to select, * selects author first, and if not present, editor, translator or collaborator, in that order. '*' '*' | 'author' | 'collaborator' | 'editor' | 'translator' initials add author initials false boolean authAuthEa(creator: '*' | 'author' | 'collaborator' | 'editor' | 'translator'='*', initials: boolean=false, sep: string='.')The last name of the first two authors, and ".ea" if there are more than two.
parameter value assumed when not provided valid values creator kind of creator to select, * selects author first, and if not present, editor, translator or collaborator, in that order. '*' '*' | 'author' | 'collaborator' | 'editor' | 'translator' initials add author initials false boolean sep use this character between authors '.' string authEtAl(creator: '*' | 'author' | 'collaborator' | 'editor' | 'translator'='*', initials: boolean=false, sep: string=' ')The last name of the first author, and the last name of the second author if there are two authors or "EtAl" if there are more than two. This is similar to authEtal2. The difference is that the authors are not separated by "." and in case of more than 2 authors "EtAl" instead of ".etal" is appended.
parameter value assumed when not provided valid values creator kind of creator to select, * selects author first, and if not present, editor, translator or collaborator, in that order. '*' '*' | 'author' | 'collaborator' | 'editor' | 'translator' initials add author initials false boolean sep use this character between authors ' ' string authEtal2(creator: '*' | 'author' | 'collaborator' | 'editor' | 'translator'='*', initials: boolean=false, sep: string='.')The last name of the first author, and the last name of the second author if there are two authors or ".etal" if there are more than two.
parameter value assumed when not provided valid values creator kind of creator to select, * selects author first, and if not present, editor, translator or collaborator, in that order. '*' '*' | 'author' | 'collaborator' | 'editor' | 'translator' initials add author initials false boolean sep use this character between authors '.' string authForeIni(creator: '*' | 'author' | 'collaborator' | 'editor' | 'translator'='*')The given-name initial of the first author.
parameter value assumed when not provided valid values creator kind of creator to select, * selects author first, and if not present, editor, translator or collaborator, in that order. '*' '*' | 'author' | 'collaborator' | 'editor' | 'translator' authIni(n: number=0, creator: '*' | 'author' | 'collaborator' | 'editor' | 'translator'='*', initials: boolean=false, sep: string='.')The beginning of each author's last name, using no more than n characters (0 = all).
parameter value assumed when not provided valid values n the number of characters to take from the name, 0 = all 0 number creator kind of creator to select, * selects author first, and if not present, editor, translator or collaborator, in that order. '*' '*' | 'author' | 'collaborator' | 'editor' | 'translator' initials add author initials false boolean sep use this character between authors '.' string authorIni(creator: '*' | 'author' | 'collaborator' | 'editor' | 'translator'='*', initials: boolean=false, sep: string='.')The first 5 characters of the first author's last name, and the last name initials of the remaining authors.
parameter value assumed when not provided valid values creator kind of creator to select, * selects author first, and if not present, editor, translator or collaborator, in that order. '*' '*' | 'author' | 'collaborator' | 'editor' | 'translator' initials add author initials false boolean sep use this character between authors '.' string authorLast(creator: '*' | 'author' | 'collaborator' | 'editor' | 'translator'='*', initials: boolean=false)The last name of the last author
parameter value assumed when not provided valid values creator kind of creator to select, * selects author first, and if not present, editor, translator or collaborator, in that order. '*' '*' | 'author' | 'collaborator' | 'editor' | 'translator' initials add author initials false boolean authorLastForeIni(creator: '*' | 'author' | 'collaborator' | 'editor' | 'translator'='*')The given-name initial of the last author.
parameter value assumed when not provided valid values creator kind of creator to select, * selects author first, and if not present, editor, translator or collaborator, in that order. '*' '*' | 'author' | 'collaborator' | 'editor' | 'translator' authorsAlpha(creator: '*' | 'author' | 'collaborator' | 'editor' | 'translator'='*', initials: boolean=false, sep: string=' ')Corresponds to the BibTeX style "alpha". One author: First three letters of the last name. Two to four authors: First letters of last names concatenated. More than four authors: First letters of last names of first three authors concatenated. "+" at the end.
parameter value assumed when not provided valid values creator kind of creator to select, * selects author first, and if not present, editor, translator or collaborator, in that order. '*' '*' | 'author' | 'collaborator' | 'editor' | 'translator' initials add author initials false boolean sep use this character between authors ' ' string authorsn(n: number=0, creator: '*' | 'author' | 'collaborator' | 'editor' | 'translator'='*', initials: boolean=false, sep: string=' ')The last names of the first n (default: all) authors.
parameter value assumed when not provided valid values n the number of characters to take from the name, 0 = all 0 number creator kind of creator to select, * selects author first, and if not present, editor, translator or collaborator, in that order. '*' '*' | 'author' | 'collaborator' | 'editor' | 'translator' initials add author initials false boolean sep use this character between authors ' ' string authshort(creator: '*' | 'author' | 'collaborator' | 'editor' | 'translator'='*', initials: boolean=false, sep: string='.')The last name if one author/editor is given; the first character of up to three authors' last names if more than one author is given. A plus character is added, if there are more than three authors.
parameter value assumed when not provided valid values creator kind of creator to select, * selects author first, and if not present, editor, translator or collaborator, in that order. '*' '*' | 'author' | 'collaborator' | 'editor' | 'translator' initials add author initials false boolean sep use this character between authors '.' string creatornames(template?: string containing at least one of '%(F)s', '%(f)s', '%(g)s', '%(i)s', '%(I)s', '%(g_zh)s', '%(f_zh)s', transliterate?: boolean, space?: string)Sets the defaults for formatting creator names in the creator template, you can use:
%(f)s: family ("last") name %(F)s: family ("last") name without dropping particles %(f_zh)s: family ("last") name extracted from chinese compound names. Need jieba to be enabled %(g)s: given ("first") name %(g_zh)s: given ("first") name extracted from chinese compound names. Need jieba to be enabled %(i)s: given-name initials %(I)s: given-name initials, upper-case parameter value assumed when not provided valid values template sprintf-template default for representing creator names. Default is '%(f)s'. undefined string containing at least one of '%(F)s', '%(f)s', '%(g)s', '%(i)s', '%(I)s', '%(g_zh)s', '%(f_zh)s' transliterate transliterate the name undefined boolean space replace spaces in the name (in case of multi-part names for example) with this string undefined string creators(n: number | [ number, number ]=0, type: 'artist' | 'attorneyAgent' | 'author' | 'bookAuthor' | 'cartographer' | 'castMember' | 'commenter' | 'composer' | 'contributor' | 'cosponsor' | 'counsel' | 'director' | 'editor' | 'guest' | 'interviewee' | 'interviewer' | 'inventor' | 'performer' | 'podcaster' | 'presenter' | 'producer' | 'programmer' | 'recipient' | 'reviewedAuthor' | 'scriptwriter' | 'seriesEditor' | 'sponsor' | 'translator' | 'wordsBy' | ('artist' | 'attorneyAgent' | 'author' | 'bookAuthor' | 'cartographer' | 'castMember' | 'commenter' | 'composer' | 'contributor' | 'cosponsor' | 'counsel' | 'director' | 'editor' | 'guest' | 'interviewee' | 'interviewer' | 'inventor' | 'performer' | 'podcaster' | 'presenter' | 'producer' | 'programmer' | 'recipient' | 'reviewedAuthor' | 'scriptwriter' | 'seriesEditor' | 'sponsor' | 'translator' | 'wordsBy')[] | (('artist' | 'attorneyAgent' | 'author' | 'bookAuthor' | 'cartographer' | 'castMember' | 'commenter' | 'composer' | 'contributor' | 'cosponsor' | 'counsel' | 'director' | 'editor' | 'guest' | 'interviewee' | 'interviewer' | 'inventor' | 'performer' | 'podcaster' | 'presenter' | 'producer' | 'programmer' | 'recipient' | 'reviewedAuthor' | 'scriptwriter' | 'seriesEditor' | 'sponsor' | 'translator' | 'wordsBy' | '*')[])[] | '*'=[ [ 'primary', 'editor', 'translator', '*' ] ], name: string containing at least one of '%(F)s', '%(f)s', '%(g)s', '%(i)s', '%(I)s', '%(g_zh)s', '%(f_zh)s' | ''='', etal: string='', sep: string=' ', min: number=0, max: number=0)Author/editor information. in the creator template, you can use:
%(f)s: family ("last") name %(F)s: family ("last") name without dropping particles %(f_zh)s: family ("last") name extracted from chinese compound names. Need jieba to be enabled %(g)s: given ("first") name %(g_zh)s: given ("first") name extracted from chinese compound names. Need jieba to be enabled %(i)s: given-name initials %(I)s: given-name initials, upper-case parameter value assumed when not provided valid values n select the first n creators (when passing a number) or the authors in this range (inclusive, when passing two values); negative numbers mean "from the end", default = 0 = all 0 number | [ number, number ] type select only creators of given type(s). Default: all [ [ 'primary', 'editor', 'translator', '*' ] ] 'artist' | 'attorneyAgent' | 'author' | 'bookAuthor' | 'cartographer' | 'castMember' | 'commenter' | 'composer' | 'contributor' | 'cosponsor' | 'counsel' | 'director' | 'editor' | 'guest' | 'interviewee' | 'interviewer' | 'inventor' | 'performer' | 'podcaster' | 'presenter' | 'producer' | 'programmer' | 'recipient' | 'reviewedAuthor' | 'scriptwriter' | 'seriesEditor' | 'sponsor' | 'translator' | 'wordsBy' | ('artist' | 'attorneyAgent' | 'author' | 'bookAuthor' | 'cartographer' | 'castMember' | 'commenter' | 'composer' | 'contributor' | 'cosponsor' | 'counsel' | 'director' | 'editor' | 'guest' | 'interviewee' | 'interviewer' | 'inventor' | 'performer' | 'podcaster' | 'presenter' | 'producer' | 'programmer' | 'recipient' | 'reviewedAuthor' | 'scriptwriter' | 'seriesEditor' | 'sponsor' | 'translator' | 'wordsBy')[] | (('artist' | 'attorneyAgent' | 'author' | 'bookAuthor' | 'cartographer' | 'castMember' | 'commenter' | 'composer' | 'contributor' | 'cosponsor' | 'counsel' | 'director' | 'editor' | 'guest' | 'interviewee' | 'interviewer' | 'inventor' | 'performer' | 'podcaster' | 'presenter' | 'producer' | 'programmer' | 'recipient' | 'reviewedAuthor' | 'scriptwriter' | 'seriesEditor' | 'sponsor' | 'translator' | 'wordsBy' | '*')[])[] | '*' name sprintf-js template. Available named parameters are: f (family name), F (family name without dropping particle), g (given name), i (initials) '' string containing at least one of '%(F)s', '%(f)s', '%(g)s', '%(i)s', '%(I)s', '%(g_zh)s', '%(f_zh)s' | '' etal use this term to replace authors after n authors have been named '' string sep use this character between authors ' ' string min skip to the next pattern if there are less than min creators, 0 = ignore 0 number max skip to the next pattern if there are more than max creators, 0 = ignore 0 number creatortypes(match?: RegExp)This will return a comma-separated list of creator type information for all creators on the item in the form <1 or 2><creator-type>, where 1 or 2 denotes a 1-part or 2-part creator, and creator-type is one of {{% citekey-formatters/creatortypes %}}, or primary for the primary creator-type of the Zotero item under consideration. The list is prefixed by the item type, so might look like audioRecording:2performer,2performer,1composer.
parameter value assumed when not provided valid values match Regex to test the creator-type list. When passed, and the creator-type list does not match the regex, jump to the next formule. When it matches, return nothing but stay in the current formule. When no regex is passed, output the creator-type list for the item (mainly useful for debugging). undefined RegExp date(format: string='%Y-%m-%d')The date of the publication
parameter value assumed when not provided valid values format sprintf-style format template '%Y-%m-%d' string extra(variable: string)A pseudo-field from the extra field. eg if you have Original date: 1970 in your extra field, you can get it as extra(originalDate), or tex.shortauthor: APA which you could get with extra('tex.shortauthor'). Any tex. field will be picked up, the other fields can be selected from this list of key names.
parameter value assumed when not provided valid values variable extra-field line identifier none; must be provided string firstpageThe number of the first page of the publication (Caution: this will return the lowest number found in the pages field, since BibTeX allows 7,41,73--97 or 43+.)
group(name: string)Tests whether the item is in the given group library
parameter value assumed when not provided valid values name group name none; must be provided string infix(format: string containing at least one of '%(a)s', '%(A)s', '%(n)s'='%(a)s', start: number | string=0)a pseudo-function that sets the citekey disambiguation infix using an sprintf-js format spec for when a key is generated that already exists. The infix charachter appears at the place of this function of the formula instead of at the and (as postfix does). You must include exactly one of the placeholders %(n)s (number), %(a)s (alpha, lowercase) or %(A)s (alpha, uppercase). For the rest of the disambiguator you can use things like padding and extra text as sprintf-js allows. With start set to 1 the disambiguator is always included, even if there is no need for it when no duplicates exist. The default format is %(a)s. in the template, you can use:
%(a)s: lower-case alphabetic disambiguator %(A)s: upper-case alphabetic disambiguator %(n)s: numeric disambiguator parameter value assumed when not provided valid values format sprintf-style format template '%(a)s' string containing at least one of '%(a)s', '%(A)s', '%(n)s' start start value for postfix 0 number | string inspireHepFetches the key from inspire-hep based on DOI or arXiv ID
item(id: 'id' | 'key'='key')returns the internal item ID/key
parameter value assumed when not provided valid values id 'id': return itemID; 'key': return the item key 'key' 'id' | 'key' journal(abbrev: 'abbrev' | 'abbrev+auto' | 'auto' | 'full' | 'off'='abbrev+auto')returns the journal abbreviation, or, if not found, the journal title, If 'automatic journal abbreviation' is enabled in the BBT settings, it will use the same abbreviation filter Zotero uses in the wordprocessor integration. You might want to use the abbr filter on this. Abbreviation behavior can be specified as abbrev+auto (the default) which uses the explicit journal abbreviation if present, and tries the automatic abbreviator if not (if auto-abbrev is enabled in the preferences), auto (skip explicit journal abbreviation even if present), abbrev (no auto-abbrev even if it is enabled in the preferences) or full/off (no abbrevation).
parameter value assumed when not provided valid values abbrev abbreviation mode 'abbrev+auto' 'abbrev' | 'abbrev+auto' | 'auto' | 'full' | 'off' keyword(n: number)Tag number n. Mostly for legacy compatibility
parameter value assumed when not provided valid values n position of tag to get none; must be provided number language(name...: ('alemannic' | 'ame' | 'american' | 'american english' | 'americanenglish' | 'ar' | 'ar-ar' | 'ar-dz' | 'ar-eg' | 'ar-iq' | 'ar-jo' | 'ar-lb' | 'ar-ma' | 'ar-ps' | 'ar-sa' | 'ar-sy' | 'ar-tn' | 'ara' | 'arabic' | 'arabic-algeria' | 'arabic-dz' | 'arabic-eg' | 'arabic-egypt' | 'arabic-iq' | 'arabic-iraq' | 'arabic-jo' | 'arabic-jordan' | 'arabic-lb' | 'arabic-lebanon' | 'arabic-ma' | 'arabic-morocco' | 'arabic-palestinianterritories' | 'arabic-ps' | 'arabic-sa' | 'arabic-saudiarabia' | 'arabic-sy' | 'arabic-syria' | 'arabic-tn' | 'arabic-tunisia' | 'australian' | 'australian english' | 'australianenglish' | 'austrian' | 'austrian german' | 'austrian-traditional' | 'austriangerman' | 'austriangerman-traditional' | 'bri' | 'british' | 'british english' | 'britishenglish' | 'canadian' | 'canadian english' | 'canadianenglish' | 'chinese' | 'chinese-hans' | 'chinese-hans-hk' | 'chinese-hans-mo' | 'chinese-hans-sg' | 'chinese-hant' | 'chinese-hant-hk' | 'chinese-hant-mo' | 'chinese-pinyin' | 'chinese-simplified' | 'chinese-simplified-hongkongsarchina' | 'chinese-simplified-macausarchina' | 'chinese-simplified-singapore' | 'chinese-traditional' | 'chinese-traditional-hongkongsarchina' | 'chinese-traditional-macausarchina' | 'de' | 'de-1901' | 'de-1996' | 'de-at' | 'de-at-1901' | 'de-at-1996' | 'de-ch' | 'de-ch-1901' | 'de-ch-1996' | 'de-de' | 'de-de-1901' | 'deutsch' | 'en' | 'en-au' | 'en-ca' | 'en-en' | 'en-gb' | 'en-nz' | 'en-us' | 'eng' | 'english' | 'english-au' | 'english-australia' | 'english-ca' | 'english-canada' | 'english-gb' | 'english-newzealand' | 'english-nz' | 'english-unitedkingdom' | 'english-unitedstates' | 'english-us' | 'ger' | 'german' | 'german-at' | 'german-at-traditional' | 'german-austria' | 'german-austria-traditional' | 'german-ch' | 'german-ch-traditional' | 'german-de' | 'german-de-traditional' | 'german-germany' | 'german-germany-traditional' | 'german-switzerland' | 'german-switzerland-traditional' | 'german-traditional' | 'gsw' | 'hànyǔ pīnyīn' | 'ja' | 'ja-ja' | 'jap' | 'japanese' | 'nau' | 'naustrian' | 'newzealand' | 'nge' | 'ngerman' | 'nsw' | 'nswissgerman' | 'österreichisches deutsch' | 'pinyin' | 'schweizer hochdeutsch' | 'schwiizertüütsch' | 'simplified chinese' | 'swi' | 'swiss german' | 'swiss high german' | 'swissgerman' | 'swisshighgerman' | 'swisshighgerman-traditional' | 'traditional chinese' | 'tw' | 'ukenglish' | 'usenglish' | 'zh' | 'zh-hans' | 'zh-hans-hk' | 'zh-hans-mo' | 'zh-hans-sg' | 'zh-hant' | 'zh-hant-hk' | 'zh-hant-mo' | 'zh-latn-pinyin' | 'zh-tw' | 'zh-zh' | 'العربية' | '中文' | '中文-中文' | '日本語' | '简体中文' | '繁體中文')[])Tests whether the item has the given language set, and skips to the next pattern if not
parameter value assumed when not provided valid values name... one or more language codes none; must be provided 'alemannic' | 'ame' | 'american' | 'american english' | 'americanenglish' | 'ar' | 'ar-ar' | 'ar-dz' | 'ar-eg' | 'ar-iq' | 'ar-jo' | 'ar-lb' | 'ar-ma' | 'ar-ps' | 'ar-sa' | 'ar-sy' | 'ar-tn' | 'ara' | 'arabic' | 'arabic-algeria' | 'arabic-dz' | 'arabic-eg' | 'arabic-egypt' | 'arabic-iq' | 'arabic-iraq' | 'arabic-jo' | 'arabic-jordan' | 'arabic-lb' | 'arabic-lebanon' | 'arabic-ma' | 'arabic-morocco' | 'arabic-palestinianterritories' | 'arabic-ps' | 'arabic-sa' | 'arabic-saudiarabia' | 'arabic-sy' | 'arabic-syria' | 'arabic-tn' | 'arabic-tunisia' | 'australian' | 'australian english' | 'australianenglish' | 'austrian' | 'austrian german' | 'austrian-traditional' | 'austriangerman' | 'austriangerman-traditional' | 'bri' | 'british' | 'british english' | 'britishenglish' | 'canadian' | 'canadian english' | 'canadianenglish' | 'chinese' | 'chinese-hans' | 'chinese-hans-hk' | 'chinese-hans-mo' | 'chinese-hans-sg' | 'chinese-hant' | 'chinese-hant-hk' | 'chinese-hant-mo' | 'chinese-pinyin' | 'chinese-simplified' | 'chinese-simplified-hongkongsarchina' | 'chinese-simplified-macausarchina' | 'chinese-simplified-singapore' | 'chinese-traditional' | 'chinese-traditional-hongkongsarchina' | 'chinese-traditional-macausarchina' | 'de' | 'de-1901' | 'de-1996' | 'de-at' | 'de-at-1901' | 'de-at-1996' | 'de-ch' | 'de-ch-1901' | 'de-ch-1996' | 'de-de' | 'de-de-1901' | 'deutsch' | 'en' | 'en-au' | 'en-ca' | 'en-en' | 'en-gb' | 'en-nz' | 'en-us' | 'eng' | 'english' | 'english-au' | 'english-australia' | 'english-ca' | 'english-canada' | 'english-gb' | 'english-newzealand' | 'english-nz' | 'english-unitedkingdom' | 'english-unitedstates' | 'english-us' | 'ger' | 'german' | 'german-at' | 'german-at-traditional' | 'german-austria' | 'german-austria-traditional' | 'german-ch' | 'german-ch-traditional' | 'german-de' | 'german-de-traditional' | 'german-germany' | 'german-germany-traditional' | 'german-switzerland' | 'german-switzerland-traditional' | 'german-traditional' | 'gsw' | 'hànyǔ pīnyīn' | 'ja' | 'ja-ja' | 'jap' | 'japanese' | 'nau' | 'naustrian' | 'newzealand' | 'nge' | 'ngerman' | 'nsw' | 'nswissgerman' | 'österreichisches deutsch' | 'pinyin' | 'schweizer hochdeutsch' | 'schwiizertüütsch' | 'simplified chinese' | 'swi' | 'swiss german' | 'swiss high german' | 'swissgerman' | 'swisshighgerman' | 'swisshighgerman-traditional' | 'traditional chinese' | 'tw' | 'ukenglish' | 'usenglish' | 'zh' | 'zh-hans' | 'zh-hans-hk' | 'zh-hans-mo' | 'zh-hans-sg' | 'zh-hant' | 'zh-hant-hk' | 'zh-hant-mo' | 'zh-latn-pinyin' | 'zh-tw' | 'zh-zh' | 'العربية' | '中文' | '中文-中文' | '日本語' | '简体中文' | '繁體中文' lastpageThe number of the last page of the publication (See the remark on firstpage)
...
pinnedLegacy compatibility formatter.
Pinned citation keys in Extra are no longer supported, but existing formulas may still reference $pinned(). Keep this formatter as a no-op.
postfix(format: string containing at least one of '%(a)s', '%(A)s', '%(n)s'='%(a)s', start: number | string=0)a pseudo-function that sets the citekey disambiguation postfix using an sprintf-js format spec for when a key is generated that already exists. Does not add any text to the citekey otherwise. You must include exactly one of the placeholders %(n)s (number), %(a)s (alpha, lowercase) or %(A)s (alpha, uppercase). For the rest of the disambiguator you can use things like padding and extra text as sprintf-js allows. With start set to 1 the disambiguator is always included, even if there is no need for it when no duplicates exist. The default format is %(a)s. in the template, you can use:
%(a)s: lower-case alphabetic disambiguator %(A)s: upper-case alphabetic disambiguator %(n)s: numeric disambiguator parameter value assumed when not provided valid values format sprintf-style format template '%(a)s' string containing at least one of '%(a)s', '%(A)s', '%(n)s' start start value for postfix 0 number | string shorttitle(n: number=3, m: number=0)The first n (default: 3) words of the title, apply capitalization to first m (default: 0) of those.
parameter value assumed when not provided valid values n number of words to select 3 number m number of words to capitalize. 0 means no words will be capitalized. Mind that existing capitals are not removed. 0 number shortyearThe last 2 digits of the publication year
...
transliterate(mode: 'ar' | 'arabic' | 'bg' | 'bulgarian' | 'chinese' | 'chinese-traditional' | 'de' | 'german' | 'ja' | 'japanese' | 'macedonian' | 'mg' | 'minimal' | 'mk' | 'mongolian' | 'rs' | 'ru' | 'russian' | 'serbian' | 'tw' | 'uk' | 'ukranian' | 'zh' | 'zh-hant')Set the default transliteration mode. If you don't specify a mode, the mode for an entry is derived from the item language field
parameter value assumed when not provided valid values mode specialized translateration modes for german, japanese or chinese. none; must be provided 'ar' | 'arabic' | 'bg' | 'bulgarian' | 'chinese' | 'chinese-traditional' | 'de' | 'german' | 'ja' | 'japanese' | 'macedonian' | 'mg' | 'minimal' | 'mk' | 'mongolian' | 'rs' | 'ru' | 'russian' | 'serbian' | 'tw' | 'uk' | 'ukranian' | 'zh' | 'zh-hant' type(allowed...: ('annotation' | 'artwork' | 'attachment' | 'audioRecording' | 'bill' | 'blogPost' | 'book' | 'bookSection' | 'case' | 'computerProgram' | 'conferencePaper' | 'dataset' | 'dictionaryEntry' | 'document' | 'email' | 'encyclopediaArticle' | 'film' | 'forumPost' | 'hearing' | 'instantMessage' | 'interview' | 'journalArticle' | 'letter' | 'magazineArticle' | 'manuscript' | 'map' | 'newspaperArticle' | 'note' | 'patent' | 'podcast' | 'preprint' | 'presentation' | 'radioBroadcast' | 'report' | 'standard' | 'statute' | 'thesis' | 'tvBroadcast' | 'videoRecording' | 'webpage')[])Without arguments, returns the item type. When arguments as passed, tests whether the item is of any of the given types, and skips to the next pattern if not, eg type(book) + veryshorttitle | auth + year.
parameter value assumed when not provided valid values allowed... one or more item type names none; must be provided 'annotation' | 'artwork' | 'attachment' | 'audioRecording' | 'bill' | 'blogPost' | 'book' | 'bookSection' | 'case' | 'computerProgram' | 'conferencePaper' | 'dataset' | 'dictionaryEntry' | 'document' | 'email' | 'encyclopediaArticle' | 'film' | 'forumPost' | 'hearing' | 'instantMessage' | 'interview' | 'journalArticle' | 'letter' | 'magazineArticle' | 'manuscript' | 'map' | 'newspaperArticle' | 'note' | 'patent' | 'podcast' | 'preprint' | 'presentation' | 'radioBroadcast' | 'report' | 'standard' | 'statute' | 'thesis' | 'tvBroadcast' | 'videoRecording' | 'webpage' veryshorttitle(n: number=1, m: number=0)The first n words of the title, apply capitalization to first m of those
parameter value assumed when not provided valid values n number of words to select 1 number m number of words to capitalize. 0 means no words will be capitalized. Mind that existing capitals are not removed. 0 number yearThe year of the publication
...
Direct access to unprocessed fields (“Field functions”) The above functions all retrieve information stored in the item’s fields and process it in some way. If you don’t want this, you can instead call field contents without any processing. To access Zotero fields, refer to them as given in the table below:
...
parameter value assumed when not provided valid values chars number of characters to return per word 1 number .acronym(list: string='acronyms', reload: boolean=false, passthrough: boolean=false)Does an acronym lookup for the text.
parameter value assumed when not provided valid values list lookup list. The list must be a CSV file and live in the Zotero/better-bibtex directory in your Zotero profile, and must use commas as the delimiter. 'acronyms' string reload reload the list for every call. When off, the list will only be read at startup of Better BibTeX. You can set this to true temporarily to live-reload a list. false boolean passthrough if no match is found, pass through input. false boolean .alphanumclears out everything but unicode alphanumeric characters (unicode character classes L and N)
.asciiremoves all non-ascii characters
.capitalize(wordstart: RegExp=/((?<=^|\\s)[a-z])/g)uppercases the first letter of each word
parameter value assumed when not provided valid values wordstart /((?<=^|\\s)[a-z])/g RegExp .cleantransliterates the citation key and removes unsafe characters
...
parameter value assumed when not provided valid values sep replacement character '' string .default(text: string)Returns the given text if no output was generated
parameter value assumed when not provided valid values text literal text to return none; must be provided string .discarddiscards the input
.find(match: RegExp | string, passthrough: boolean=false)Finds a text in the string and returns it.
parameter value assumed when not provided valid values match regex or string to match. String matches are case-insensitive none; must be provided RegExp | string passthrough if no match is found, pass through input. false boolean .formatDate(format: string='%Y-%m-%d')formats date as by replacing y, m and d in the format
parameter value assumed when not provided valid values format sprintf-style format template '%Y-%m-%d' string .ideographsTreat ideaographs as individual words
.jiebaword segmentation for Chinese items. Uses substantial memory, and adds about 7 seconds to BBTs startup time; must be enabled under Preferences -> Better BibTeX -> Advanced -> Citekeys
...
parameter value assumed when not provided valid values relation comparison operator '>' '!=' | '<' | '<=' | '==' | '>' | '>=' length value to compare length with 0 number .localTimetransforms date/time to local time. Mainly useful for dateAdded and dateModified as it requires an ISO-formatted input.
...
.match(match: RegExp | string, clean: boolean=false)If the output does not match the given string/regex, skip to the next pattern.
parameter value assumed when not provided valid values match regex or string to match. String matches are case-insensitive none; must be provided RegExp | string clean transliterates the current output and removes unsafe characters during matching false boolean .nopunct(dash: string='-')Removes punctuation
parameter value assumed when not provided valid values dash replace dashes with given character '-' string .nopunctordashRemoves punctuation and word-connecting dashes. alias for nopunct(dash='')
...
parameter value assumed when not provided valid values postfix postfix string none; must be provided string .prefix(prefix: string)prefixes with its parameter, so .prefix('_') will add an underscore to the front if, and only if, the value it is supposed to prefix isn't empty.
parameter value assumed when not provided valid values prefix prefix string none; must be provided string .replace(find: string | RegExp, replace: string)replaces text, for the text to match you can pass either:
a string: .replace('.etal','&etal') which will match case-insensitive, so will replace .EtAl with &etal. javascript regular expression: .replace(/[.]etal/ig, '&etal') parameter value assumed when not provided valid values find string or regex to match. String matches are case-insensitive none; must be provided string | RegExp replace literal text to replace the match with none; must be provided string .select(start: number=1, n?: number)selects words from the value passed in. The format is select(start,number) (1-based), so select(1,4) or select(n=4) would select the first four words. If n is not given, all words from start to the end are selected.
parameter value assumed when not provided valid values start first word to select (1-based) 1 number n number of words to select. Default is all. undefined number .skipwords(nopunct: boolean=false)filters out common words like 'of', 'the', … the list of words can be seen and changed by going into about:config under the key extensions.zotero.translators.better-bibtex.skipWords as a comma-separated, case-insensitive list of words.
...
parameter value assumed when not provided valid values nopunct remove punctuation from words false boolean .substring(start: number=1, n?: number)substring(start,n) selects n (default: all) characters starting at start
parameter value assumed when not provided valid values start starting character (counting from 1) 1 number n number of characters to select (default: remainder from start) undefined number .transliterate(mode?: 'ar' | 'arabic' | 'bg' | 'bulgarian' | 'chinese' | 'chinese-traditional' | 'de' | 'german' | 'ja' | 'japanese' | 'macedonian' | 'mg' | 'minimal' | 'mk' | 'mongolian' | 'rs' | 'ru' | 'russian' | 'serbian' | 'tw' | 'uk' | 'ukranian' | 'zh' | 'zh-hant')transliterates the input. If you don't specify a mode, the mode is derived from the item language field
parameter value assumed when not provided valid values mode specialized translateration modes for german, japanese or chinese. undefined 'ar' | 'arabic' | 'bg' | 'bulgarian' | 'chinese' | 'chinese-traditional' | 'de' | 'german' | 'ja' | 'japanese' | 'macedonian' | 'mg' | 'minimal' | 'mk' | 'mongolian' | 'rs' | 'ru' | 'russian' | 'serbian' | 'tw' | 'uk' | 'ukranian' | 'zh' | 'zh-hant' .upperForces the text inserted by the field marker to be in uppercase. For example, auth.upper expands the last name of the first author in uppercase.
Usage note: the functions condense, skipwords, capitalize and select rely on whitespaces for word handling. Most functions strip whitespace and thereby make these filter functions sort of useless. You will in general want to use the fields from the table above, which give you the values from Zotero without any changes. The fields with ** are only available in Juris-M.`,description:`Note: a number of functions below talk about the author’s lastname; you can read that as “when available”. If you have the name as a single-field name (for entities like International Business Machines or Aristoteles), Zotero doesn’t have a last name, and the full single-field name is taken instead.
Functions auth(n: number=0, m: number=1, creator: '*' | 'author' | 'collaborator' | 'editor' | 'translator'='*', initials: boolean=false)The first n (default: all) characters of the mth (default: first) author's last name.`,tags:["Citation Keys"],title:"Formulas",uri:"/zotero-better-bibtex/citing/formulas/index.html"},{breadcrumb:"Better BibTeX for Zotero > Tags",content:"",description:"",tags:[],title:"Tag :: Git",uri:"/zotero-better-bibtex/tags/git/index.html"},{breadcrumb:"Better BibTeX for Zotero > Tags",content:"",description:"",tags:[],title:"Tag :: Import",uri:"/zotero-better-bibtex/tags/import/index.html"},{breadcrumb:"Better BibTeX for Zotero > Tags",content:"",description:"",tags:[],title:"Tag :: Libreoffice",uri:"/zotero-better-bibtex/tags/libreoffice/index.html"},{breadcrumb:"Better BibTeX for Zotero > Tags",content:"",description:"",tags:[],title:"Tag :: Overleaf",uri:"/zotero-better-bibtex/tags/overleaf/index.html"},{breadcrumb:"Better BibTeX for Zotero > Tags",content:"",description:"",tags:[],title:"Tag :: Scripting",uri:"/zotero-better-bibtex/tags/scripting/index.html"},{breadcrumb:"Better BibTeX for Zotero",content:`While the development needs of BBT are to a large extent covered by the generosity towards open-source developers of services such as github, my development system does require the occasional upgrade; also, I enjoy getting the occasional frivolous tech-toy that I wouldn’t otherwise grant myself. While you should feel in no way obligated to pay for BBT, anything you can spare is very much appreciated. If you’d rather contribute a little bit each month (and a little means a lot) so I can save up for a replacement a year or so down the line, head on over to Patreon, but mind that Patreon takes a fairly large cut of what you give.
...
I prefer communicating via github issues, but for private information you can reach me on matrix`,description:"While the development needs of BBT are to a large extent covered by the generosity towards open-source developers of services such as github, my development system does require the occasional upgrade; also, I enjoy getting the occasional frivolous tech-toy that I wouldn’t otherwise grant myself. While you should feel in no way obligated to pay for BBT, anything you can spare is very much appreciated. If you’d rather contribute a little bit each month (and a little means a lot) so I can save up for a replacement a year or so down the line, head on over to Patreon, but mind that Patreon takes a fairly large cut of what you give.",tags:[],title:"Sponsoring BBT",uri:"/zotero-better-bibtex/sponsoring/index.html"},{breadcrumb:"Better BibTeX for Zotero",content:"",description:"",tags:[],title:"Tags",uri:"/zotero-better-bibtex/tags/index.html"},{breadcrumb:"Better BibTeX for Zotero > Tags",content:"",description:"",tags:[],title:"Tag :: Word",uri:"/zotero-better-bibtex/tags/word/index.html"}]